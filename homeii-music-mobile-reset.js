...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _build() {
    ...
    const compactTileHtml = `
      <div class="compact-shell premium-player-tile">
        ...
        <div class="compact-top-row">
          <div class="compact-mode-badge">${this._m("Music flow", "זרימת מוזיקה")}</div>
          <div class="compact-top-actions">
            ${playerFocusHtml}
            <button class="compact-expand-btn" id="compactExpandBtn" title="${this._m("Expand player", "הרחב נגן")}">${this._iconSvg("menu")}</button>
          </div>
        </div>
        <div class="compact-body">
          <div class="compact-cover-wrap">
            <div class="compact-cover-echo" id="compactCoverAura"></div>
            <div class="compact-cover-glass"></div>
            <button class="compact-cover" id="npArt" title="${this._m("Expand player", "הרחב נגן")}"></button>
          </div>
          <div class="compact-main">
            <div class="compact-copy">
              <div class="compact-kicker">${this._m("Now playing", "מתנגן עכשיו")}</div>
            ...
            <div class="progress-line compact-progress-line">
            ...
            <div class="compact-controls-row">
              ...
              <div class="compact-expand-copy">${this._m("Open full player", "פתח נגן מלא")}</div>
            </div>
            <div class="mobile-volume-inline compact-volume-inline">
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host { display:block; width:100%; max-width:100%; overflow:hidden; --mobile-height:${height}px; --v2-font-scale:${this._state.mobileFontScale || 1}; --v2-custom-rgb:${this._customRgb()}; --v2-custom-text:${this._customTextColor()}; --accent-color:${this._state.mobileCustomColor || "#e0a11b"}; --ma-accent: var(--accent-color, #e0a11b); }
        * { box-sizing:border-box; }
        .card {
        ...
        .compact-backdrop-shade {
          background:
            ...
            linear-gradient(180deg, rgba(5,8,14,.1), rgba(5,8,14,.72) 42%, rgba(5,8,14,.92));
        }
        .compact-sheen {
          background:
        ...
        .compact-top-actions .player-focus {
          margin:0;
          max-width:none;
          justify-content:flex-start;
          min-width:0;
        ...
        .compact-cover-glass {
          inset:0;
          background:
            linear-gradient(145deg, rgba(255,255,255,.18), rgba(255,255,255,.03) 34%, rgba(255,255,255,.08) 100%);
        ...
        .compact-progress-line .progress {
          min-width:0;
        }
        ...
        .theme-light .compact-shell {
          border-color:rgba(124,140,162,.22);
          background:
            linear-gradient(180deg, rgba(255,255,255,.82), rgba(233,238,246,.94)),
            rgba(241,245,251,.92);
          box-shadow:
            0 24px 52px rgba(96,113,136,.18),
            inset 0 1px 0 rgba(255,255,255,.88);
        }
        .theme-light .compact-backdrop-art {
          opacity:.34;
          filter:blur(36px) saturate(1.12) brightness(1.08);
        }
        .theme-light .compact-backdrop-shade {
          background:
            linear-gradient(135deg, rgba(255,255,255,.62), rgba(255,255,255,.08) 34%),
            linear-gradient(180deg, rgba(255,255,255,.08), rgba(232,238,246,.66) 42%, rgba(224,231,241,.92));
        }
        .theme-light .compact-sheen {
          background:
            radial-gradient(circle at 16% 22%, rgba(241,186,83,.28), transparent 24%),
            radial-gradient(circle at 82% 18%, rgba(255,255,255,.5), transparent 16%),
            radial-gradient(circle at 56% 100%, rgba(225,163,49,.18), transparent 28%);
        }
        .theme-light .compact-mode-badge,
        .theme-light .compact-top-actions .player-focus,
        .theme-light .compact-volume-inline,
        .theme-light .compact-expand-btn {
          background:rgba(255,255,255,.66);
          border-color:rgba(124,140,162,.18);
          color:#213043;
        }
        .theme-light .compact-cover-glass {
          background:
            linear-gradient(145deg, rgba(255,255,255,.78), rgba(255,255,255,.14) 34%, rgba(255,255,255,.34) 100%);
          border-color:rgba(124,140,162,.22);
          box-shadow:inset 0 1px 0 rgba(255,255,255,.92);
        }
        .theme-light .compact-expand-copy {
          color:#667487;
        }
        ...
        .card.compact-mode .player-focus {
          margin-top:10px;
          min-height:32px;
          padding:6px 10px;
          max-width:min(64vw, 180px);
        }
        .card.compact-mode.compact-collapsed .compact-top-actions .player-focus {
          margin-top:0;
          max-width:none;
        }
        .card.compact-mode .player-focus-name {
          font-size:13px !important;
        }
        .card.compact-mode .player-focus-art-wrap {
...
.theme-light .player-mini-value{color:#435066!important;}
  ...
  _syncNowPlayingUI() {
    const player = this._getSelectedPlayer();
    const compactTileMode = this._isCompactTileMode();
    const renderCompactTile = ({ title = "", subtitle = "", art = "", icon = "music_note", duration = 0, position = 0 }) => {
      ...
      if (artHost) {
        artHost.classList.toggle("placeholder", !art);
        artHost.style.backgroundImage = art ? `url("${this._esc(art)}")` : "";
        artHost.innerHTML = art ? "" : this._iconSvg(icon);
      }
      if (compactBackdrop) compactBackdrop.style.backgroundImage = art ? `url("${this._esc(art)}")` : "";
      if (compactCoverAura) compactCoverAura.style.backgroundImage = art ? `url("${this._esc(art)}")` : "";
      if (bg) bg.style.backgroundImage = art ? `${overlay}, url("${this._esc(art)}")` : "";
...
if (!customElements.get("homeii-music-flow")) {
  customElements.define("homeii-music-flow", MABrowserCardMobile);
}

if (!customElements.get("homeii-music-mobile")) {
  customElements.define("homeii-music-mobile", MABrowserCardMobile);
}
...
if (!customElements.get("homeii-music-flow-editor")) {
  customElements.define("homeii-music-flow-editor", MABrowserCardMobileEditor);
}

if (!customElements.get("homeii-music-mobile-editor")) {
  customElements.define("homeii-music-mobile-editor", MABrowserCardMobileEditor);
}

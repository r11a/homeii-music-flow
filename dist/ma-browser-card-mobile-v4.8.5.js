﻿﻿class MABrowserCard extends HTMLElement {
  constructor() {
    ...
    this._state = {
      ...
      smartShelfMode: "queue",
      smartShelfForYouItems: [],
      smartShelfRenderedHtml: "",
...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _tabletStabilityModeEnabled() {
    try {
      ...
      return this._layoutModeConfig() === "tablet" && /Android/i.test(ua) && width >= 900 && touchPoints > 0;
    } catch (_) {
      return false;
    }
  }
  ...
  _handleWindowResize() {
    ...
    this._resizeTimer = setTimeout(() => {
      ...
      const previousTablet = previousWidth >= 900;
      const currentTablet = currentWidth >= 900;
      ...
      if (previousTablet === currentTablet && widthDelta < widthThreshold && heightDelta < heightThreshold) {
...
const HOMEII_CARD_VERSION = "4.8.5";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _applyConfiguredMobileSettings() {
    ...
    if (this._usesVisualSettings()) {
      ...
      this._state.mobileBackgroundMotionMode = ["off", "subtle", "strong", "extreme"].includes(String(cfg.mobile_background_motion_mode || "").toLowerCase()) ? String(cfg.mobile_background_motion_mode).toLowerCase() : "subtle";
      this._state.mobileCustomTextTone = String(cfg.mobile_custom_text_tone || "light") === "dark" ? "dark" : "light";
      this._state.mobileFontScale = Math.max(0.5, Math.min(1.5, Number(cfg.mobile_font_scale || 1) || 1));
      ...
      this._state.mobileStudioShortcutEnabled = cfg.mobile_studio_shortcut !== false;
      ...
      this._state.mobileHomeShortcutPath = String(cfg.mobile_home_shortcut_path || "/").trim() || "/";
      ...
      const pinnedEntities = Array.isArray(cfg.pinned_player_entities) && cfg.pinned_player_entities.length
        ? cfg.pinned_player_entities
        : (String(cfg.pinned_player_entity || "").trim() ? [String(cfg.pinned_player_entity || "").trim()] : []);
      this._state.pinnedPlayerEntities = pinnedEntities
  ...
  _sleepTimerStartedFromNightMode() {
    return this._sleepTimerRemainingMs() > 0 && String(this._state.mobileSleepTimerOrigin || "") === "night";
  }

  _sleepTimerChipVisible() {
    return this._sleepTimerStartedFromNightMode();
  }
  ...
  _tabletAutoFitEnabled() {
    return this._layoutModeConfig() === "tablet";
  }

  _tabletAutoFitDense(showNightRow = this._mobileNightMode() !== "off", showUpNext = false) {
    return this._tabletAutoFitEnabled() && !!(showNightRow || showUpNext);
  }

  _syncTabletAutoFitUi(hasUpNext = null) {
    const card = this.shadowRoot?.querySelector(".card");
    if (!card) return;
    const autoFit = this._tabletAutoFitEnabled();
    ...
    card.classList.toggle("tablet-auto-fit", autoFit);
    card.classList.toggle("tablet-fit-night", autoFit && showNightRow);
    card.classList.toggle("tablet-fit-up-next", autoFit && upNextVisible);
    card.classList.toggle("tablet-fit-dense", autoFit && (showNightRow || upNextVisible));
  }
  ...
  _setSleepTimerMinutes(minutes = 15, source = "general") {
    ...
    this._state.mobileSleepTimerEndsAt = Date.now() + (amount * 60000);
    this._state.mobileSleepTimerPlayer = player.entity_id;
    this._state.mobileSleepTimerOrigin = String(source || "").toLowerCase() === "night" ? "night" : "";
    this._state.mobileSleepTimerMenuOpen = false;
    this._persistMobileAppearance();
    this._syncNightModeUi();
    this._syncSleepTimerChip();
  ...
  _addSleepTimerMinutes(minutes = 15) {
    const amount = Math.max(1, Number(minutes) || 0);
    const base = this._sleepTimerRemainingMs() ? Number(this._state.mobileSleepTimerEndsAt || 0) : Date.now();
    const player = this._getSelectedPlayer();
    this._state.mobileSleepTimerEndsAt = Math.max(base, Date.now()) + (amount * 60000);
  ...
  _cycleSleepTimer(source = "general") {
    ...
    if (!currentRemaining) {
      ...
      this._state.mobileSleepTimerPlayer = player.entity_id;
      this._state.mobileSleepTimerOrigin = normalizedSource === "night" ? "night" : "";
      this._state.mobileSleepTimerMenuOpen = false;
      this._persistMobileAppearance();
      this._syncNightModeUi();
      this._syncSleepTimerChip();
    ...
    if (normalizedSource === "night") this._state.mobileSleepTimerOrigin = "night";
    this._state.mobileSleepTimerMenuOpen = false;
    this._persistMobileAppearance();
    this._syncNightModeUi();
    this._syncSleepTimerChip();
  ...
  _mobileHomeShortcutPath() {
    const raw = String(this._state.mobileHomeShortcutPath || "/").trim();
    if (!raw) return "/";
    if (raw.startsWith("/")) return raw;
    return `/${raw.replace(/^\/+/, "")}`;
  }
  ...
  _pinnedPlayerPreferences() {
    const source = Array.isArray(this._state.pinnedPlayerEntities)
      ? this._state.pinnedPlayerEntities
      : [];
    const next = [];
    source.forEach((entityId) => {
  ...
  _build() {
    ...
    const dockVolumeHtml = layoutMode === "tablet" || volumeMode === "always" ? volumeHtml : ``;
    const dockVolumeButtonHtml = volumeMode === "button" && layoutMode !== "tablet"
      ? `<button class="side-btn" id="controlVolumeBtn">${this._iconSvg("volume_high")}</button>`
      : ``;
    const smartShelfHtml = compactTileMode ? `` : `<div class="smart-shelf" id="smartShelf" hidden></div>`;
        ...
        <div class="media-dock" id="mediaDock">
          <div class="media-dock-inner">
            ...
            <div class="controls">
              ...
              ${dockVolumeButtonHtml}
              <button class="side-btn minor-btn" id="mobileRepeatBtn">${this._iconSvg("repeat")}</button>
            </div>
            ${dockVolumeHtml ? `<div class="media-dock-volume">${dockVolumeHtml}</div>` : ``}
          </div>
        </div>
        ${smartShelfHtml}
      </div>`;
...
.player-sub { margin-top:1px; font-size:calc(11px * var(--v2-font-scale)); color:rgba(255,255,255,.72); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        ...
        .bottom { display:grid; width:100%; max-width:100%; min-width:0; gap:16px; align-content:end; }
        ...
        .media-dock {
          ...
          overflow:hidden;
          border-radius:34px;
          background:linear-gradient(180deg, rgba(19,24,36,.34), rgba(9,13,21,.24));
          border:1px solid rgba(255,255,255,.12);
          box-shadow:
            0 24px 44px rgba(0,0,0,.2),
            inset 0 1px 0 rgba(255,255,255,.08),
        ...
        .media-dock::before {
          ...
          border-radius:inherit;
          background:
            radial-gradient(circle at 50% -6%, rgba(255,255,255,.12), transparent 44%),
            linear-gradient(180deg, rgba(255,255,255,.08), transparent 32%);
          pointer-events:none;
        }
        .theme-light .media-dock {
          background:linear-gradient(180deg, rgba(255,255,255,.56), rgba(236,242,249,.3));
          border-color:rgba(143,159,181,.18);
          box-shadow:
            0 22px 42px rgba(96,112,134,.12),
            inset 0 1px 0 rgba(255,255,255,.82),
            inset 0 -1px 0 rgba(161,177,199,.08);
        }
        .theme-light .media-dock::before {
          background:
            radial-gradient(circle at 50% -6%, rgba(255,255,255,.64), transparent 40%),
        ...
        .card.layout-tablet .media-dock-inner {
          gap:18px;
          padding:20px clamp(22px, 2.8vw, 34px) 22px;
        }
        .media-dock .progress-line,
        .media-dock .controls,
        .media-dock .mobile-volume-inline {
          width:100%;
          margin-inline:0;
        }
        .media-dock .progress-line {
        ...
        .media-dock .progress-time {
          min-width:58px;
          font-size:14px;
          font-weight:800;
        }
        .card.layout-tablet .media-dock .progress-time {
          font-size:15px;
          min-width:62px;
        }
        .media-dock .progress {
        ...
        .theme-light .media-dock .progress {
          background:linear-gradient(180deg, rgba(106,120,140,.14), rgba(106,120,140,.09));
          box-shadow:
            inset 0 1px 0 rgba(255,255,255,.62),
            inset 0 0 0 1px rgba(113,128,150,.08),
            inset 0 -1px 2px rgba(93,108,129,.08);
        }
        .media-dock .controls {
        ...
        .theme-light .media-dock .side-btn,
        .theme-light .media-dock .main-btn,
        .theme-light .media-dock .volume-btn {
          background:linear-gradient(160deg, rgba(255,255,255,.9), rgba(239,244,250,.66));
          border-color:rgba(143,159,181,.2);
          box-shadow:
            0 16px 28px rgba(96,112,134,.12),
            inset 0 1px 0 rgba(255,255,255,.9);
        }
        .card.layout-tablet .media-dock .side-btn,
        .card.layout-tablet .media-dock .volume-btn {
          width:64px;
          height:64px;
        }
        .card.layout-tablet .media-dock .side-btn.minor-btn {
          width:54px;
          height:54px;
        }
        .card.layout-tablet .media-dock .main-btn {
          width:108px;
          height:108px;
        }
        .media-dock-volume {
        ...
        .media-dock .volume-value {
          min-width:62px;
          font-size:14px;
          font-weight:900;
        }
        .media-dock .mobile-volume-inline .volume-btn {
          width:62px;
          height:62px;
        }
        .media-dock .volume-slider {
          height:10px;
          background:linear-gradient(to right,var(--ma-accent) 0%,var(--ma-accent) var(--vol-pct,50%),rgba(255,255,255,.16) var(--vol-pct,50%),rgba(255,255,255,.16) 100%);
          box-shadow:
            inset 0 1px 0 rgba(255,255,255,.08),
        ...
        .theme-light .media-dock .volume-slider {
          background:linear-gradient(to right,var(--ma-accent) 0%,var(--ma-accent) var(--vol-pct,50%),rgba(94,110,132,.14) var(--vol-pct,50%),rgba(94,110,132,.14) 100%);
          box-shadow:
            inset 0 1px 0 rgba(255,255,255,.56),
            inset 0 -1px 1px rgba(94,110,132,.08);
        }
        .media-dock .volume-slider::-webkit-slider-thumb {
          width:20px;
          height:20px;
          border:2px solid rgba(255,255,255,.72);
          box-shadow:0 8px 16px rgba(0,0,0,.16);
        }
        .media-dock .volume-slider::-moz-range-thumb {
          width:20px;
          height:20px;
          border:2px solid rgba(255,255,255,.72);
          box-shadow:0 8px 16px rgba(0,0,0,.16);
        ...
        .smart-shelf {
          ...
          padding:18px clamp(18px, 3vw, 28px) 20px;
          border-radius:30px;
          background:linear-gradient(180deg, rgba(12,16,24,.2), rgba(12,16,24,.12));
          border:1px solid rgba(255,255,255,.08);
          box-shadow:0 18px 34px rgba(0,0,0,.12), inset 0 1px 0 rgba(255,255,255,.06);
          backdrop-filter:blur(18px) saturate(1.04);
          -webkit-backdrop-filter:blur(18px) saturate(1.04);
        }
        .theme-light .smart-shelf {
          background:linear-gradient(180deg, rgba(255,255,255,.42), rgba(238,243,250,.24));
          border-color:rgba(143,159,181,.14);
          box-shadow:0 16px 30px rgba(96,112,134,.1), inset 0 1px 0 rgba(255,255,255,.82);
        }
        .smart-shelf-head {
          display:flex;
          align-items:center;
        ...
        .smart-shelf-tab:hover:not(:disabled) {
          transform:translateY(-1px);
        }
        .smart-shelf-tab.active {
          background:color-mix(in srgb, var(--ma-accent) 24%, rgba(255,255,255,.08));
          border-color:color-mix(in srgb, var(--ma-accent) 34%, rgba(255,255,255,.1));
          box-shadow:0 12px 24px color-mix(in srgb, var(--ma-accent) 16%, transparent);
        }
        .smart-shelf-tab:disabled {
          opacity:.42;
          cursor:default;
        }
        .theme-light .smart-shelf-tab {
        ...
        .theme-light .smart-shelf-caption {
          color:#5e6e84;
        }
        .smart-shelf-track {
          display:flex;
        ...
        .smart-shelf-track::-webkit-scrollbar {
          display:none;
        }
        .smart-shelf-card {
          min-width:220px;
          ...
          border:1px solid rgba(255,255,255,.1);
          box-shadow:0 16px 28px rgba(0,0,0,.14), inset 0 1px 0 rgba(255,255,255,.06);
          backdrop-filter:blur(16px);
          -webkit-backdrop-filter:blur(16px);
          cursor:pointer;
        ...
        .smart-shelf-card:hover {
          transform:translateY(-2px);
          border-color:color-mix(in srgb, var(--ma-accent) 28%, rgba(255,255,255,.12));
        }
        .smart-shelf-card:active,
        .smart-shelf-card.pressed {
          transform:translateY(1px) scale(.985);
        }
        .theme-light .smart-shelf-card {
          background:linear-gradient(160deg, rgba(255,255,255,.88), rgba(240,245,251,.58));
          border-color:rgba(143,159,181,.16);
          box-shadow:0 16px 28px rgba(96,112,134,.12), inset 0 1px 0 rgba(255,255,255,.88);
        }
        .smart-shelf-art {
          width:56px;
          height:56px;
          ...
          display:grid;
          place-items:center;
          background:linear-gradient(135deg, color-mix(in srgb, var(--ma-accent) 18%, transparent), rgba(255,255,255,.06));
          border:1px solid rgba(255,255,255,.12);
        ...
        .theme-light .smart-shelf-kicker {
          color:#6f7f95;
        }
        .theme-light .smart-shelf-title {
          color:#1f2633;
        }
        .theme-light .smart-shelf-sub {
          color:#617189;
        }
        .smart-shelf-card.skeleton {
          pointer-events:none;
        }
        .smart-shelf-card.skeleton .smart-shelf-art,
        .smart-shelf-card.skeleton .smart-shelf-line {
          position:relative;
          overflow:hidden;
          background:rgba(255,255,255,.08);
        }
        .theme-light .smart-shelf-card.skeleton .smart-shelf-art,
        .theme-light .smart-shelf-card.skeleton .smart-shelf-line {
          background:rgba(119,136,158,.12);
        }
        .smart-shelf-card.skeleton .smart-shelf-art::after,
        .smart-shelf-card.skeleton .smart-shelf-line::after {
          content:"";
          position:absolute;
          inset:0;
          transform:translateX(-100%);
          background:linear-gradient(90deg, transparent, rgba(255,255,255,.18), transparent);
          animation:smart-shelf-shimmer 1.2s ease-in-out infinite;
        }
        .smart-shelf-lines {
          display:grid;
          gap:8px;
        ...
        .smart-shelf-line.long {
          width:88%;
        }
        .smart-shelf-line.short {
          width:62%;
        }
        @keyframes smart-shelf-shimmer {
        ...
        .theme-light .smart-shelf-empty {
          color:#607089;
        }
        .card.layout-tablet .smart-shelf {
          padding:18px clamp(22px, 2.8vw, 34px) 22px;
        }
        .footer-nav {
          display:flex;
          align-items:stretch;
...
@media (max-width: 1280px){
  ...
  _hideSmartShelf(clear = false) {
    const host = this.$("smartShelf");
    if (!host) return;
    host.hidden = true;
    if (clear) {
      host.innerHTML = "";
      this._state.smartShelfRenderedHtml = "";
    }
  }

  _smartShelfLabel(mode = "queue") {
    if (mode === "recent") return this._m("Recent", "אחרונים");
    if (mode === "for_you") return this._m("For You", "בשבילך");
    return this._m("Up Next", "הבא בתור");
  }

  _smartShelfCaption(mode = "queue") {
    if (mode === "recent") return this._m("Jump back into something you just played.", "חזרה מהירה למשהו שנוגן ממש עכשיו.");
    if (mode === "for_you") return this._m("Curated quick picks for the room.", "בחירות מהירות ונעימות לחדר.");
    return this._m("Move through the current queue from here.", "דילוג מהיר בין הפריטים הבאים בתור.");
  }

  _smartShelfQueueItems(limit = 5) {
    const queueState = this._state.maQueueState || {};
    const items = Array.isArray(this._state.queueItems) ? [...this._state.queueItems] : [];
    if (!items.length) return [];
    const ordered = items
    ...
    let nextItems = [];
    if (Number.isFinite(currentIndex)) {
      nextItems = ordered.filter((item) => Number(item?.sort_index) > currentIndex);
    } else {
      const currentKey = this._getQueueItemKey(queueState.current_item);
      const currentPos = currentKey ? ordered.findIndex((item) => this._getQueueItemKey(item) === currentKey) : -1;
      nextItems = currentPos >= 0 ? ordered.slice(currentPos + 1) : ordered.slice(1);
    }
    return nextItems.slice(0, limit).map((item) => {
      const mediaItem = item?.media_item || {};
      const title = this._queueItemPrimaryTitle(item) || this._m("Up next", "הבא בתור");
      const subtitle = [
        this._queueItemPrimaryArtist(item),
        mediaItem?.album?.name || item?.album || "",
      ].filter(Boolean).join(" · ") || this._emptyQuickSuggestionLabel(mediaItem?.media_type || item?.media_type || "track");
      return {
        title,
        subtitle,
        image: this._queueItemImageUrl(item, 160) || mediaItem?.image || mediaItem?.album?.image || "",
        uri: this._getQueueItemUri(item),
        media_type: mediaItem?.media_type || item?.media_type || "track",
        queue_item_id: this._getQueueItemKey(item),
        sort_index: Number.isFinite(Number(item?.sort_index)) ? String(item.sort_index) : "",
        kicker: this._smartShelfLabel("queue"),
        action: "queue",
      };
    }).filter((item) => item.uri || item.queue_item_id);
  }

  _smartShelfRecentItems(limit = 5) {
    return this._visibleRecentHistoryItems().slice(0, limit).map((item) => ({
      title: item?.title || this._m("Recent track", "שיר קודם"),
      subtitle: item?.artist || item?.album || item?.provider_label || "—",
      image: item?.image || "",
      uri: item?.uri || "",
      media_type: item?.media_type || "track",
      kicker: this._smartShelfLabel("recent"),
      action: "media",
    })).filter((item) => item.uri);
  }

  _smartShelfCardHtml(item = {}, index = 0) {
    const title = item?.title || item?.name || this._m("Quick play", "ניגון מהיר");
    const subtitle = item?.subtitle || item?.artist || item?.album || "—";
    const mediaType = item?.media_type || "track";
    const art = item?.image || "";
    const action = item?.action || "media";
    return `
      <button
        class="smart-shelf-card"
        data-smart-shelf-action="${this._esc(action)}"
        data-smart-shelf-uri="${this._esc(item?.uri || "")}"
        data-smart-shelf-type="${this._esc(mediaType)}"
        data-smart-shelf-queue-item-id="${this._esc(item?.queue_item_id || "")}"
        data-smart-shelf-sort-index="${this._esc(item?.sort_index || "")}"
        data-smart-shelf-index="${this._esc(String(index))}"
        title="${this._esc(title)}"
      >
        <span class="smart-shelf-art">${art ? `<img src="${this._esc(art)}" alt="">` : this._iconSvg(mediaType === "radio" ? "radio" : mediaType === "playlist" ? "queue" : "music")}</span>
        <span class="smart-shelf-copy">
          <span class="smart-shelf-kicker">${this._esc(item?.kicker || this._emptyQuickSuggestionLabel(mediaType))}</span>
          <span class="smart-shelf-title">${this._esc(title)}</span>
          <span class="smart-shelf-sub">${this._esc(subtitle)}</span>
        </span>
      </button>
    `;
  }

  async _ensureSmartShelfForYouItems() {
    if (Array.isArray(this._state.smartShelfForYouItems) && this._state.smartShelfForYouItems.length) {
      return this._state.smartShelfForYouItems;
    }
    if (this._smartShelfForYouPromise) return this._smartShelfForYouPromise;
    const promise = (async () => {
      const results = await Promise.allSettled([
        this._fetchLibrary("playlist", "sort_name", 18, true),
        this._fetchLibrary("playlist", "sort_name", 36, false),
        this._fetchLibrary("album", "random", 18, false),
        this._fetchLibrary("radio", "sort_name", 10, true),
      ]);
      ...
      pushUnique(this._pickRandomItems(likedPlaylists, 2));
      pushUnique(this._pickRandomItems(playlists.filter((item) => (item?.media_type || "") === "playlist"), 2));
      pushUnique(this._pickRandomItems(albums, 2));
      pushUnique(this._pickRandomItems(radios, 1));
      if (unique.length < 6) pushUnique(this._pickRandomItems([...playlists, ...albums, ...radios], 6 - unique.length));
      this._state.smartShelfForYouItems = unique.slice(0, 6);
      return this._state.smartShelfForYouItems;
    })();
    this._smartShelfForYouPromise = promise;
    try {
      return await promise;
    } finally {
      if (this._smartShelfForYouPromise === promise) this._smartShelfForYouPromise = null;
    }
  }

  async _renderSmartShelf() {
    const host = this.$("smartShelf");
    const card = this.shadowRoot?.querySelector(".card");
    if (!host || !card || this._isCompactTileMode() || card.classList.contains("empty-media")) {
      this._hideSmartShelf();
      return;
    }
    const queueItems = this._smartShelfQueueItems(this._layoutModeConfig() === "tablet" ? 5 : 4);
    const recentItems = this._smartShelfRecentItems(this._layoutModeConfig() === "tablet" ? 5 : 4);
    let forYouItems = Array.isArray(this._state.smartShelfForYouItems) ? this._state.smartShelfForYouItems : [];
    let mode = String(this._state.smartShelfMode || "queue");
    if (!["queue", "recent", "for_you"].includes(mode)) mode = "queue";
    if (mode === "queue" && !queueItems.length) mode = recentItems.length ? "recent" : "for_you";
    if (mode === "recent" && !recentItems.length) mode = queueItems.length ? "queue" : "for_you";
    const fallbackMode = queueItems.length ? "queue" : recentItems.length ? "recent" : "for_you";
    if (!queueItems.length && !recentItems.length && !forYouItems.length) mode = "for_you";
    if (mode !== this._state.smartShelfMode && (queueItems.length || recentItems.length || forYouItems.length)) {
      this._state.smartShelfMode = mode;
    }
    const shouldLoadForYou = mode === "for_you" || (!queueItems.length && !recentItems.length);
    const loadingForYou = shouldLoadForYou && !forYouItems.length;
    if (loadingForYou) {
      this._ensureSmartShelfForYouItems()
        .then(() => this._renderSmartShelf().catch(() => {}))
        .catch(() => {
          this._state.smartShelfForYouItems = [];
          this._renderSmartShelf().catch(() => {});
        });
    }
    forYouItems = Array.isArray(this._state.smartShelfForYouItems) ? this._state.smartShelfForYouItems : [];
    const activeMode = mode === "for_you" && forYouItems.length ? "for_you" : mode || fallbackMode;
    const items = activeMode === "queue"
      ? queueItems
      : activeMode === "recent"
        ? recentItems
        : forYouItems;
    const hasVisibleContent = queueItems.length || recentItems.length || forYouItems.length || loadingForYou;
    if (!hasVisibleContent) {
      this._hideSmartShelf(true);
      return;
    }
    const tabs = [
      { mode: "queue", label: this._smartShelfLabel("queue"), disabled: !queueItems.length },
      { mode: "recent", label: this._smartShelfLabel("recent"), disabled: !recentItems.length },
      { mode: "for_you", label: this._smartShelfLabel("for_you"), disabled: false },
    ];
    const bodyHtml = loadingForYou && activeMode === "for_you"
      ? Array.from({ length: this._layoutModeConfig() === "tablet" ? 4 : 3 }, (_, index) => `
          <div class="smart-shelf-card skeleton" aria-hidden="true" data-smart-shelf-skeleton="${index}">
            <span class="smart-shelf-art"></span>
            <span class="smart-shelf-lines">
              <span class="smart-shelf-line long"></span>
              <span class="smart-shelf-line short"></span>
            </span>
          </div>
        `).join("")
      : items.length
        ? items.map((item, index) => this._smartShelfCardHtml({
            ...item,
            kicker: item?.kicker || (activeMode === "for_you" ? this._emptyQuickSuggestionLabel(item?.media_type || "album") : this._smartShelfLabel(activeMode)),
            image: item?.image || this._artUrl(item) || item?.media_item?.image || item?.media_item?.album?.image || "",
            title: item?.title || item?.name || this._m("Quick play", "ניגון מהיר"),
            subtitle: item?.subtitle || item?.artist || item?.album || item?.provider_label || "—",
          }, index)).join("")
        : `<div class="smart-shelf-empty">${this._esc(this._m("Play something and this shelf will start helping.", "הפעל משהו והמדף הזה יתחיל לעזור."))}</div>`;
    const nextHtml = `
      <div class="smart-shelf-head">
        <div class="smart-shelf-tabs">
          ${tabs.map((tab) => `
            <button class="smart-shelf-tab ${activeMode === tab.mode ? "active" : ""}" data-smart-shelf-mode="${this._esc(tab.mode)}" ${tab.disabled ? "disabled" : ""}>
              ${this._esc(tab.label)}
            </button>
          `).join("")}
        </div>
        <div class="smart-shelf-caption">${this._esc(this._smartShelfCaption(activeMode))}</div>
      </div>
      <div class="smart-shelf-track">
        ${bodyHtml}
      </div>
    `;
    host.hidden = false;
    if (this._state.smartShelfRenderedHtml === nextHtml) return;
    host.innerHTML = nextHtml;
    this._state.smartShelfRenderedHtml = nextHtml;
    host.querySelectorAll("[data-smart-shelf-mode]").forEach((btn) => btn.addEventListener("click", () => {
      if (btn.disabled) return;
      this._state.smartShelfMode = btn.dataset.smartShelfMode || "queue";
      this._renderSmartShelf().catch(() => {});
    }));
    host.querySelectorAll("[data-smart-shelf-action]").forEach((btn) => btn.addEventListener("click", async (e) => {
      const trigger = e.currentTarget;
      const action = trigger.dataset.smartShelfAction || "media";
      const uri = trigger.dataset.smartShelfUri || "";
      const mediaType = trigger.dataset.smartShelfType || "track";
      if (!uri && action !== "queue") return;
      if (!this._pressUiButton(trigger)) return;
      if (action === "queue") {
        await this._playQueueItem(
          trigger.dataset.smartShelfQueueItemId || "",
          uri,
          mediaType,
          trigger.dataset.smartShelfSortIndex || "",
        );
        return;
      }
      await this._playMedia(uri, mediaType, "play", {
        label: trigger.getAttribute("title") || "",
        sourceEl: trigger,
      });
    }));
  }

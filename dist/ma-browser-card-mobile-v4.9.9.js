...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _getThisDevicePlayer(players = this._state.players || []) {
    return HomeiiPlayersFoundation.getThisDevicePlayer(players, this._getRememberedThisDevicePlayerId());
  }
  ...
  _selectPlayer(entityId, manual = false) {
    ...
    const nextEntityId = pinnedEntities.length
      ? (pinnedEntities.includes(entityId) ? entityId : pinnedEntities[0])
      : entityId;
    if (!nextEntityId) return;
    if (manual && pinnedEntities.length && entityId && !pinnedEntities.includes(entityId)) {
  ...
  _sourceProviderMeta(value = "") {
    return HomeiiMediaPresentationFoundation.sourceProviderMeta(value, {
      libraryLabel: this._m("Library", "ספריה"),
      radioLabel: this._m("Radio", "רדיו"),
    });
  }
  ...
  _syncRecentHistoryUi() {
    ...
    if (!this._getSelectedPlayer()) {
      if (this._state.mobileHistoryRenderedHtml !== "") {
        host.innerHTML = "";
    ...
    const nextHtml = `
      ...
      `).join("")}
    `;
    const changed = this._state.mobileHistoryRenderedHtml !== nextHtml;
    if (changed) {
  ...
  _playerByEntityId(entityId = "") {
    return HomeiiPlayersFoundation.playerByEntityId(entityId, this._state.players || [], this._hass?.states || {});
  }
  ...
  _controlRoomGridStyle(playerCount = 0) {
    ...
    const cardRect = this.shadowRoot?.querySelector(".card")?.getBoundingClientRect?.() || null;
    const width = Math.max(960, Math.round(cardRect?.width || this._lastViewportWidth || window.innerWidth || 1600));
    const compactViewport = width <= 1280;
    const gap = compactViewport ? 14 : 18;
    let cols = Math.min(count, compactViewport ? 3 : 4);
    if (count <= 1) cols = 1;
    else if (count === 2) cols = 2;
    else if (count === 3) cols = width >= 1480 ? 3 : 2;
    else if (count === 4) cols = 2;
    else if (count <= 6) cols = 3;
    else cols = compactViewport ? 3 : 4;
    return `style="--control-room-cols:${cols};--control-room-gap:${gap}px;--control-room-player-count:${count}"`;
  }
  ...
  _controlRoomRenderSignature() {
    const players = this._controlRoomPlayers().map((player) => ({
      ...
      volume: Math.round((player.attributes?.volume_level || 0) * 100),
      muted: this._isMuted(player),
    }));
      ...
      <div class="control-room-scene ${primaryArt ? "has-art" : ""}" ${sceneStyle}>
        ...
        <div class="control-room-layout">
          <div class="control-room-grid-wrap">
            <div class="control-room-grid" ${gridStyle}>
      ...
      <div class="lyrics-sheet">
        <div class="lyrics-head">
          ...
          <div class="lyrics-head-actions">
            <button class="lyrics-sync-btn ${this._state.mobileLyricsSyncEnabled !== false ? "active" : ""}" id="lyricsSyncBtn" title="${this._esc(this._m("Sync lyrics", "סנכרון מילים"))}">
              ${this._iconSvg("sync")}
              <span>${this._esc(this._m("Sync", "סנכרון"))}</span>
            </button>
  ...
  _currentMediaLikeMeta() {
    return HomeiiFavoritesFoundation.buildCurrentMediaLikeMeta({
      ...
      fallbackName: this._m("Unknown", "לא ידוע"),
  ...
  _handleWindowResize() {
    ...
    this._resizeTimer = setTimeout(() => {
      ...
      const previousWidth = this._lastViewportWidth || 0;
      const previousHeight = this._lastViewportHeight || 0;
      ...
      if (resizeStrategy.keyboardLikeResize) return;
      if (resizeStrategy.softSync) {
  ...
  _loadPlayers() {
    ...
    if (pinnedEntities.length) {
      if (!pinnedEntities.includes(this._state.selectedPlayer)) this._state.selectedPlayer = pinnedEntities[0];
      this._state.hasAutoSelectedPlayer = true;
    } else {
  ...
  _normalizeMediaItem(item) {
    return HomeiiMediaPresentationFoundation.normalizeMediaItem(item, this._maUrl);
  }

  _imageProxyUrl(path, provider = "", size = 300) {
    return HomeiiMediaPresentationFoundation.imageProxyUrl(path, provider, size, this._maUrl);
  }
...
const HOMEII_CARD_VERSION = "4.9.9";
...
const HomeiiEditorLocale = Object.freeze({
  ...
  detectEditorHebrew(doc = globalThis.document) {
    try {
...
const HomeiiConfigValidators = Object.freeze({
  assertStringIfDefined(value, key) {
    if (value == null) return;
    if (typeof value !== "string") throw new Error(`${key} must be a string`);
  },
  assertBooleanIfDefined(value, key) {
    if (value == null) return;
    if (typeof value !== "boolean") throw new Error(`${key} must be a boolean`);
  },
  assertNumberIfDefined(value, key) {
    if (value == null) return;
  ...
  assertValueInList(value, key, allowedValues) {
    if (value == null) return;
  ...
  validateBaseCardEditorConfig(config) {
    ...
    HomeiiConfigValidators.assertValueInList(config.theme_mode, "theme_mode", ["auto", "dark", "light"]);
  ...
  validateMobileCardEditorConfig(config) {
    HomeiiConfigValidators.validateBaseCardEditorConfig(config);

    HomeiiConfigValidators.assertValueInList(config.layout_mode, "layout_mode", ["auto", "mobile", "tablet"]);
    HomeiiConfigValidators.assertValueInList(config.settings_source, "settings_source", ["device", "visual", "ui", "card"]);
    HomeiiConfigValidators.assertValueInList(config.night_mode, "night_mode", ["off", "auto", "on"]);
    HomeiiConfigValidators.assertStringIfDefined(config.night_mode_auto_start, "night_mode_auto_start");
    HomeiiConfigValidators.assertStringIfDefined(config.night_mode_auto_end, "night_mode_auto_end");
    HomeiiConfigValidators.assertStringIfDefined(config.favorite_button_entity, "favorite_button_entity");
    HomeiiConfigValidators.assertBooleanIfDefined(config.allow_local_likes, "allow_local_likes");
    HomeiiConfigValidators.assertBooleanIfDefined(config.use_mass_queue_send_command, "use_mass_queue_send_command");
    HomeiiConfigValidators.assertStringIfDefined(config.mobile_custom_color, "mobile_custom_color");
    HomeiiConfigValidators.assertValueInList(config.mobile_dynamic_theme_mode, "mobile_dynamic_theme_mode", ["off", "auto", "strong"]);
    HomeiiConfigValidators.assertValueInList(config.mobile_background_motion_mode, "mobile_background_motion_mode", ["off", "subtle", "strong", "extreme"]);
    HomeiiConfigValidators.assertValueInList(config.mobile_custom_text_tone, "mobile_custom_text_tone", ["light", "dark"]);
    HomeiiConfigValidators.assertNumberIfDefined(config.mobile_font_scale, "mobile_font_scale");
    HomeiiConfigValidators.assertBooleanIfDefined(config.mobile_footer_search_enabled, "mobile_footer_search_enabled");
    HomeiiConfigValidators.assertValueInList(config.mobile_footer_mode, "mobile_footer_mode", ["icon", "text", "both"]);
    HomeiiConfigValidators.assertBooleanIfDefined(config.mobile_studio_shortcut, "mobile_studio_shortcut");
    HomeiiConfigValidators.assertBooleanIfDefined(config.mobile_home_shortcut, "mobile_home_shortcut");
    HomeiiConfigValidators.assertStringIfDefined(config.mobile_home_shortcut_path, "mobile_home_shortcut_path");
    HomeiiConfigValidators.assertValueInList(config.mobile_volume_mode, "mobile_volume_mode", ["always", "button"]);
    HomeiiConfigValidators.assertValueInList(config.mobile_mic_mode, "mobile_mic_mode", ["on", "off", "smart"]);
    HomeiiConfigValidators.assertStringArrayIfDefined(config.mobile_library_tabs, "mobile_library_tabs");
    HomeiiConfigValidators.assertStringArrayIfDefined(config.mobile_main_bar_items, "mobile_main_bar_items");
    HomeiiConfigValidators.assertValueInList(config.mobile_liked_mode, "mobile_liked_mode", ["ma", "local"]);
    HomeiiConfigValidators.assertValueInList(config.mobile_swipe_mode, "mobile_swipe_mode", ["play", "browse"]);
    HomeiiConfigValidators.assertStringIfDefined(config.mobile_radio_browser_country, "mobile_radio_browser_country");
    HomeiiConfigValidators.assertStringArrayIfDefined(config.mobile_announcement_presets, "mobile_announcement_presets");
    HomeiiConfigValidators.assertStringIfDefined(config.announcement_tts_entity, "announcement_tts_entity");
    HomeiiConfigValidators.assertBooleanIfDefined(config.mobile_compact_mode, "mobile_compact_mode");
    HomeiiConfigValidators.assertBooleanIfDefined(config.mobile_show_up_next, "mobile_show_up_next");
    HomeiiConfigValidators.assertStringIfDefined(config.pinned_player_entity, "pinned_player_entity");
    HomeiiConfigValidators.assertStringArrayIfDefined(config.pinned_player_entities, "pinned_player_entities");
  },
});

const HomeiiStateFoundation = Object.freeze({
  createBaseBrowserState() {
    return {
      view: "home",
      query: "",
      nowPlayingQuery: "",
  ...
  usesVisualSettings(config) {
    return HomeiiStateFoundation.normalizeSettingsSource(config?.settings_source) === "card";
  },
  mobileCompactModeEnabled(state) {
  ...
  backgroundMotionEnabled(state) {
    return HomeiiStateFoundation.mobileBackgroundMotionMode(state) !== "off";
  },
  backgroundMotionAmount(state) {
    const mode = HomeiiStateFoundation.mobileBackgroundMotionMode(state);
    if (mode === "extreme") return "1.85";
    if (mode === "strong") return "1.35";
    return "1";
  },
  isCompactTileMode(state) {
    return HomeiiStateFoundation.mobileCompactModeEnabled(state) && !state?.mobileCompactExpanded;
  },
});

const HomeiiMobileSettingsFoundation = Object.freeze({
  clampMobileFontScale(value) {
    return Math.max(0.5, Math.min(1.5, Number(value || 1) || 1));
  },
  normalizeEnum(value, allowedValues, fallbackValue) {
    const normalized = String(value || "").trim().toLowerCase();
  ...
  normalizeHomeShortcutPath(value, { leadingSlash = false } = {}) {
    const normalized = String(value || "/").trim() || "/";
    if (!leadingSlash) return normalized;
    if (normalized.startsWith("/")) return normalized;
    return `/${normalized.replace(/^\/+/, "")}`;
  },
  normalizeMobileFooterMode(value) {
    return HomeiiMobileSettingsFoundation.normalizeEnum(value, ["icon", "text", "both"], "both");
  },
  normalizeMobileMicMode(value) {
    return HomeiiMobileSettingsFoundation.normalizeEnum(value, ["on", "off", "smart"], "on");
  },
  normalizeMobileVolumeMode(value) {
    return HomeiiMobileSettingsFoundation.normalizeEnum(value, ["always", "button"], "button");
  },
  normalizePinnedPlayerEntityList(value) {
    const next = [];
    HomeiiMobileSettingsFoundation.normalizeStringArray(value).forEach((entityId) => {
      if (!next.includes(entityId)) next.push(entityId);
    });
    return next;
  },
  normalizePinnedPlayerEntities(config = {}) {
    const explicitEntities = HomeiiMobileSettingsFoundation.normalizeStringArray(config?.pinned_player_entities);
    if (explicitEntities.length) return HomeiiMobileSettingsFoundation.normalizePinnedPlayerEntityList(explicitEntities);
    const singleEntity = String(config?.pinned_player_entity || "").trim();
    return HomeiiMobileSettingsFoundation.normalizePinnedPlayerEntityList(singleEntity ? [singleEntity] : []);
  },
  normalizeMobileMainBarItems(items, {
    usesVisualSettings = false,
    hidePlayers = false,
    fallbackItems = [],
  } = {}) {
    const allowed = new Set(["search", "library", "players", "actions", "settings", "theme"]);
    const baseFallback = HomeiiMobileSettingsFoundation.normalizeStringArray(fallbackItems);
    const fallback = usesVisualSettings
      ? baseFallback.filter((item) => item !== "settings")
      : baseFallback;
    const source = Array.isArray(items) && items.length ? items : fallback;
    const cleaned = HomeiiMobileSettingsFoundation.normalizeStringArray(source)
  ...
  normalizeMobileLibraryTabs(tabs, fallbackTabs = []) {
    ...
    const fallback = HomeiiMobileSettingsFoundation.normalizeStringArray(fallbackTabs);
    const source = Array.isArray(tabs) && tabs.length ? tabs : fallback;
    const cleaned = HomeiiMobileSettingsFoundation.normalizeStringArray(source).filter((tab) => allowed.has(tab));
  ...
  normalizeVisualMobileState(config = {}, {
    ...
    return {
      ...
      mobileFontScale: HomeiiMobileSettingsFoundation.clampMobileFontScale(config.mobile_font_scale),
      mobileNightMode: HomeiiMobileSettingsFoundation.normalizeEnum(config.night_mode, ["off", "auto", "on"], "auto"),
      ...
      mobileFooterMode: HomeiiMobileSettingsFoundation.normalizeMobileFooterMode(config.mobile_footer_mode),
      mobileHomeShortcutEnabled: !!config.mobile_home_shortcut,
      mobileHomeShortcutPath: HomeiiMobileSettingsFoundation.normalizeHomeShortcutPath(config.mobile_home_shortcut_path),
      mobileVolumeMode: HomeiiMobileSettingsFoundation.normalizeMobileVolumeMode(config.mobile_volume_mode),
      mobileMicMode: HomeiiMobileSettingsFoundation.normalizeMobileMicMode(config.mobile_mic_mode),
      mobileLikedMode: HomeiiMobileSettingsFoundation.normalizeEnum(config.mobile_liked_mode, ["ma", "local"], "ma"),
      mobileSwipeMode: HomeiiMobileSettingsFoundation.normalizeEnum(config.mobile_swipe_mode, ["play", "browse"], "play"),
      ...
      mobileLibraryTabs: Array.isArray(config.mobile_library_tabs) && config.mobile_library_tabs.length
        ...
        : HomeiiMobileSettingsFoundation.normalizeStringArray(defaultLibraryTabs),
      mobileMainBarItems: Array.isArray(config.mobile_main_bar_items) && config.mobile_main_bar_items.length
        ...
        : HomeiiMobileSettingsFoundation.normalizeStringArray(defaultMainBarItems),
      mobileAnnouncementPresets: Array.isArray(config.mobile_announcement_presets) && config.mobile_announcement_presets.length
        ...
        : HomeiiMobileSettingsFoundation.normalizeStringArray(defaultAnnouncementPresets).slice(0, 3),
      mobileAnnouncementTtsEntity: String(config.announcement_tts_entity || ""),
      pinnedPlayerEntities: HomeiiMobileSettingsFoundation.normalizePinnedPlayerEntities(config),
    };
  },
});
...
const HomeiiResponsiveFoundation = Object.freeze({
  resolveLayoutMode(layoutMode, {
    ...
    const normalized = String(layoutMode || "auto").trim().toLowerCase();
  ...
  defaultMobileMediaLayout(layoutMode) {
    return layoutMode === "tablet" ? "grid" : "list";
  },
  tabletAutoFitEnabled(layoutMode) {
    return layoutMode === "tablet";
  },
  tabletAutoFitDense(layoutMode, {
    showNightRow = false,
    showUpNext = false,
  } = {}) {
    return HomeiiResponsiveFoundation.tabletAutoFitEnabled(layoutMode) && !!(showNightRow || showUpNext);
  },
  resolveTabletAutoFitFlags(layoutMode, {
    showNightRow = false,
    showUpNext = false,
  } = {}) {
    const autoFit = HomeiiResponsiveFoundation.tabletAutoFitEnabled(layoutMode);
    return {
      autoFit,
  ...
  resolveResizeStrategy({
    ...
    const keyboardLikeResize = HomeiiResponsiveFoundation.detectKeyboardLikeResize({
      editingText,
      widthDelta,
      heightDelta,
    });
...
const HomeiiPaletteFoundation = Object.freeze({
  ...
  normalizeRgbTuple(value, fallback = [245, 166, 35]) {
    if (Array.isArray(value) && value.length >= 3) {
      return value.slice(0, 3).map((entry) => HomeiiPaletteFoundation.clampRgbByte(entry));
    }
    if (typeof value === "string") {
      const parts = value.trim().split(/[,\s]+/).map((entry) => Number(entry));
      if (parts.length >= 3 && parts.every((entry) => Number.isFinite(entry))) {
        return parts.slice(0, 3).map((entry) => HomeiiPaletteFoundation.clampRgbByte(entry));
      }
    }
    return fallback.map((entry) => HomeiiPaletteFoundation.clampRgbByte(entry));
  },
  hexToRgbTuple(value, fallback = [245, 166, 35]) {
    const hex = String(value || "#f5a623").replace("#", "").trim();
    ...
    if (tuple.every((entry) => Number.isFinite(entry))) {
      return tuple.map((entry) => HomeiiPaletteFoundation.clampRgbByte(entry));
    }
    return HomeiiPaletteFoundation.normalizeRgbTuple(fallback);
  },
  rgbTupleToString(tuple = [245, 166, 35]) {
    const [r, g, b] = HomeiiPaletteFoundation.normalizeRgbTuple(tuple);
    return `${r} ${g} ${b}`;
  },
  rgbTupleToHex(tuple = [245, 166, 35]) {
    return `#${HomeiiPaletteFoundation.normalizeRgbTuple(tuple)
      .map((entry) => HomeiiPaletteFoundation.clampRgbByte(entry).toString(16).padStart(2, "0"))
      .join("")}`;
  },
  mixRgb(left = [245, 166, 35], right = [255, 255, 255], ratio = 0.5) {
    const weight = Math.max(0, Math.min(1, Number(ratio) || 0));
    const from = HomeiiPaletteFoundation.normalizeRgbTuple(left);
    const to = HomeiiPaletteFoundation.normalizeRgbTuple(right);
    return from.map((entry, index) => HomeiiPaletteFoundation.clampRgbByte(entry + ((to[index] - entry) * weight)));
  },
  rgbToHsl(tuple = [245, 166, 35]) {
    let [r, g, b] = HomeiiPaletteFoundation.normalizeRgbTuple(tuple).map((entry) => entry / 255);
  ...
  hslToRgb(hue = 0, saturation = 0, lightness = 0.5) {
    ...
    if (s === 0) {
      const value = HomeiiPaletteFoundation.clampRgbByte(l * 255);
    ...
    const p = (2 * l) - q;
    return [
      HomeiiPaletteFoundation.clampRgbByte(hueToRgb(p, q, h + (1 / 3)) * 255),
      HomeiiPaletteFoundation.clampRgbByte(hueToRgb(p, q, h) * 255),
      HomeiiPaletteFoundation.clampRgbByte(hueToRgb(p, q, h - (1 / 3)) * 255),
    ];
  },
  tunePaletteColor(tuple = [245, 166, 35], options = {}) {
    const [hue, saturation, lightness] = HomeiiPaletteFoundation.rgbToHsl(tuple);
    ...
    return HomeiiPaletteFoundation.hslToRgb(hue, sat, light);
  },
  isRgbTupleDark(tuple = [245, 166, 35], threshold = 0.58) {
    const [r, g, b] = HomeiiPaletteFoundation.normalizeRgbTuple(tuple);
    const luminance = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
    return luminance < Number(threshold || 0.58);
  },
  dynamicThemeStrengthValue(mode = "auto") {
    return String(mode || "").toLowerCase() === "strong" ? "1.35" : "1";
  },
  buildDynamicThemePalette({
    baseTuple = [245, 166, 35],
    vividTuple = [245, 166, 35],
    mode = "auto",
  } = {}) {
    const normalizedMode = String(mode || "auto").toLowerCase();
    const strong = normalizedMode === "strong";
    const base = HomeiiPaletteFoundation.normalizeRgbTuple(baseTuple);
    const accent = HomeiiPaletteFoundation.tunePaletteColor(vividTuple, {
      minSaturation: strong ? 0.58 : 0.46,
      minLightness: 0.42,
      maxLightness: strong ? 0.56 : 0.6,
    });
    const surface = HomeiiPaletteFoundation.mixRgb(base, accent, strong ? 0.32 : 0.2);
    const glow = HomeiiPaletteFoundation.mixRgb(accent, [255, 255, 255], strong ? 0.12 : 0.2);
    return {
      accent: HomeiiPaletteFoundation.rgbTupleToHex(accent),
      accent_rgb: HomeiiPaletteFoundation.rgbTupleToString(accent),
      surface_rgb: HomeiiPaletteFoundation.rgbTupleToString(surface),
      glow_rgb: HomeiiPaletteFoundation.rgbTupleToString(glow),
    };
  },
  resolveActiveAccentColor(palette = null, customColor = "#f5a623") {
    return palette?.accent || String(customColor || "#f5a623");
  },
  resolveActiveAccentRgb(palette = null, customColor = "#f5a623") {
    return palette?.accent_rgb || HomeiiPaletteFoundation.rgbTupleToString(HomeiiPaletteFoundation.hexToRgbTuple(customColor));
  },
});

const HomeiiNightFoundation = Object.freeze({
  normalizeNightMode(value) {
    const mode = String(value || "auto").toLowerCase();
    return ["off", "auto", "on"].includes(mode) ? mode : "auto";
  },
  normalizeClockTime(value, fallback = "22:00") {
  ...
  clockMinutesOfDay(value, fallback = "22:00") {
    const normalized = HomeiiNightFoundation.normalizeClockTime(value, fallback);
  ...
  normalizeNightModeDays(value, fallbackDays = [0, 1, 2, 3, 4, 5, 6]) {
    let source = value;
  ...
  resolveNightModeWindow(start, end, defaults = {}) {
    const fallbackStart = defaults.start || "22:00";
    const fallbackEnd = defaults.end || "06:00";
    return {
      start: HomeiiNightFoundation.normalizeClockTime(start || fallbackStart, fallbackStart),
      end: HomeiiNightFoundation.normalizeClockTime(end || fallbackEnd, fallbackEnd),
    };
  },
  isMinutesInsideWindow(minutes, startMinutes, endMinutes) {
  ...
  isNightModeActive({
    ...
    const normalizedMode = HomeiiNightFoundation.normalizeNightMode(mode);
    if (normalizedMode === "off") return false;
    if (normalizedMode === "on") return true;
    const nowMinutes = (date.getHours() * 60) + date.getMinutes();
    const startMinutes = HomeiiNightFoundation.clockMinutesOfDay(start, "22:00");
    const endMinutes = HomeiiNightFoundation.clockMinutesOfDay(end, "06:00");
    if (!HomeiiNightFoundation.isMinutesInsideWindow(nowMinutes, startMinutes, endMinutes)) return false;
    const enabledDays = new Set(HomeiiNightFoundation.normalizeNightModeDays(days));
  ...
  sleepTimerRemainingMs(targetAt, now = Date.now()) {
    const target = Number(targetAt || 0);
    if (!target || target <= now) return 0;
    return target - now;
  },
  sleepTimerRemainingLabel(remainingMs) {
  ...
  sleepTimerFooterLabel(remainingMs) {
    const remaining = Number(remainingMs || 0);
    if (!remaining) return "";
  ...
  normalizeSleepTimerOrigin(source = "") {
    return String(source || "").toLowerCase() === "night" ? "night" : "";
  },
  sleepTimerStartedFromNightMode(remainingMs, origin = "") {
    return Number(remainingMs || 0) > 0 && HomeiiNightFoundation.normalizeSleepTimerOrigin(origin) === "night";
  },
  sleepTimerChipVisible(remainingMs, origin = "") {
    return HomeiiNightFoundation.sleepTimerStartedFromNightMode(remainingMs, origin);
  },
  createSleepTimerTargetAt(minutes, now = Date.now()) {
    const amount = Math.max(1, Number(minutes) || 0);
    return Number(now || 0) + (amount * 60000);
  },
  extendSleepTimerTargetAt(existingTargetAt, minutes, now = Date.now()) {
    const amount = Math.max(1, Number(minutes) || 0);
    const base = HomeiiNightFoundation.sleepTimerRemainingMs(existingTargetAt, now)
      ? Number(existingTargetAt || 0)
      : Number(now || 0);
    return Math.max(base, Number(now || 0)) + (amount * 60000);
  },
  nextSleepTimerStep(currentRemainingMs, steps = [15, 30, 45, 60, 0]) {
...
const HomeiiMediaQueueFoundation = Object.freeze({
  ...
  parseMediaReference(uri = "", fallbackType = "track") {
  ...
  mediaRefsEquivalent(uriA = "", uriB = "", fallbackType = "track") {
    const left = String(uriA || "").trim();
    const right = String(uriB || "").trim();
    if (!left || !right) return false;
    if (left === right) return true;
    const leftRef = HomeiiMediaQueueFoundation.parseMediaReference(left, fallbackType);
    const rightRef = HomeiiMediaQueueFoundation.parseMediaReference(right, fallbackType);
    return !!(
      leftRef.provider
      && rightRef.provider
  ...
  queueItemPrimaryTitle(item = {}) {
    return item?.media_item?.name || item?.media_title || item?.name || "";
  },
  queueItemMatchTitle(item = {}) {
    return HomeiiMediaQueueFoundation.normalizeComparableText(item?.media_item?.name || item?.media_title || item?.name || "");
  },
  queueItemMatchArtist(item = {}) {
    return HomeiiMediaQueueFoundation.normalizeComparableText(
      item?.media_artist
        || (item?.media_item?.artists || []).map((artist) => artist?.name).filter(Boolean).join(", ")
        || "",
    );
  },
  queueTitleArtistMatch(item, title = "", artist = "") {
    const normalizedTitle = HomeiiMediaQueueFoundation.normalizeComparableText(title);
    if (!item || !normalizedTitle) return false;
    const itemTitle = HomeiiMediaQueueFoundation.queueItemMatchTitle(item);
    const itemArtist = HomeiiMediaQueueFoundation.queueItemMatchArtist(item);
    const normalizedArtist = HomeiiMediaQueueFoundation.normalizeComparableText(artist);
  ...
  findQueueItemIndex(queueItems = [], {
    queueItemId = "",
    uri = "",
    sortIndex = "",
    fallbackType = "track",
  } = {}, compareMediaRefs = HomeiiMediaQueueFoundation.mediaRefsEquivalent) {
    const normalizedIndex = HomeiiMediaQueueFoundation.normalizeFiniteNumber(sortIndex);
    ...
    return queueItems.findIndex((item) =>
      ...
      || (key && HomeiiMediaQueueFoundation.getQueueItemKey(item) === key)
      || (
        targetUri
        && compareMediaRefs(
          HomeiiMediaQueueFoundation.getQueueItemUri(item),
          targetUri,
  ...
  mobileCurrentQueueIndex(value) {
    const currentIndex = HomeiiMediaQueueFoundation.normalizeFiniteNumber(value);
  ...
  queueItemsContainCurrent(items = [], queueState = null) {
    if (!Array.isArray(items) || !items.length || !queueState) return false;
    const currentIndex = HomeiiMediaQueueFoundation.normalizeFiniteNumber(queueState.current_index);
    const currentKey = HomeiiMediaQueueFoundation.getQueueItemKey(queueState.current_item);
    return items.some((item) =>
      (Number.isFinite(currentIndex) && Number(item?.sort_index) === currentIndex)
      || (currentKey && HomeiiMediaQueueFoundation.getQueueItemKey(item) === currentKey)
    );
  },
  queueItemMatchesPlayer(item, player, compareMediaRefs = HomeiiMediaQueueFoundation.mediaRefsEquivalent) {
    if (!item || !player) return false;
    const playerUri = String(player?.attributes?.media_content_id || "").trim();
    const itemUri = String(HomeiiMediaQueueFoundation.getQueueItemUri(item) || "").trim();
    if (playerUri && itemUri) {
      return compareMediaRefs(playerUri, itemUri, item?.media_item?.media_type || item?.media_type || "track");
    }
    const playerTitle = HomeiiMediaQueueFoundation.normalizeComparableText(player?.attributes?.media_title || "");
    if (!playerTitle) return false;
    const playerArtist = HomeiiMediaQueueFoundation.normalizeComparableText(player?.attributes?.media_artist || "");
    return HomeiiMediaQueueFoundation.queueTitleArtistMatch(item, playerTitle, playerArtist);
  },
  resolveMobileUpNextItem(queueState = {}, queueItems = []) {
    ...
    const currentKey = HomeiiMediaQueueFoundation.getQueueItemKey(queueState.current_item);
    if (currentKey) {
      const index = items.findIndex((item) => HomeiiMediaQueueFoundation.getQueueItemKey(item) === currentKey);
      if (index >= 0 && index < items.length - 1) return items[index + 1];
    }
    return items[1] || null;
  },
  entryTargetsCurrentMedia(entry = {}, current = {}, compareMediaRefs = HomeiiMediaQueueFoundation.mediaRefsEquivalent) {
    ...
    const currentTitle = HomeiiMediaQueueFoundation.normalizeComparableText(current?.name || "");
    const entryTitle = HomeiiMediaQueueFoundation.normalizeComparableText(entry?.name || entry?.title || "");
    if (!currentTitle || !entryTitle || currentTitle !== entryTitle) return false;
    const currentArtist = HomeiiMediaQueueFoundation.normalizeComparableText(current?.artist || "");
    const entryArtist = HomeiiMediaQueueFoundation.normalizeComparableText(entry?.artist || "");
  ...
  resolveQueuePlayIndex(queueItems = [], {
    queueItemId = "",
    fallbackUri = "",
    explicitSortIndex = "",
  } = {}, compareMediaRefs = HomeiiMediaQueueFoundation.mediaRefsEquivalent) {
    const normalizedExplicit = HomeiiMediaQueueFoundation.normalizeFiniteNumber(explicitSortIndex);
  ...
  getQueueItemByIndexOrKey(queueItems = [], {
    sortIndex = "",
    queueItemId = "",
    fallbackUri = "",
  } = {}, compareMediaRefs = HomeiiMediaQueueFoundation.mediaRefsEquivalent) {
  ...
  resolveMobileArtStackContext({
    ...
    const sortedQueueItems = HomeiiMediaQueueFoundation.sortQueueItems(queueItems);
    const currentIndex = HomeiiMediaQueueFoundation.mobileCurrentQueueIndex(currentIndexValue);
    const playerUri = String(player?.attributes?.media_content_id || "").trim();
    ...
    const playerTitle = HomeiiMediaQueueFoundation.normalizeComparableText(
      hasPendingPlay
        ? (currentItemTitle || player?.attributes?.media_title || "")
        : (player?.attributes?.media_title || currentItemTitle || ""),
    );
    const playerArtist = HomeiiMediaQueueFoundation.normalizeComparableText(
      hasPendingPlay
        ? (currentItemArtist || player?.attributes?.media_artist || "")
        : (player?.attributes?.media_artist || currentItemArtist || ""),
    );
    let baseIndex = -1;
    const titleMatches = (item) => HomeiiMediaQueueFoundation.queueTitleArtistMatch(item, playerTitle, playerArtist);
    if (hasPendingPlay) {
      const pendingMatch = HomeiiMediaQueueFoundation.findQueueItemIndex(sortedQueueItems, {
    ...
    if (currentItem) {
      const currentKey = HomeiiMediaQueueFoundation.getQueueItemKey(currentItem);
      const currentUri = HomeiiMediaQueueFoundation.getQueueItemUri(currentItem);
      const keyIndex = HomeiiMediaQueueFoundation.findQueueItemIndex(sortedQueueItems, {
    ...
    if (baseIndex < 0 && playerUri) {
      baseIndex = HomeiiMediaQueueFoundation.findQueueItemIndex(sortedQueueItems, { uri: playerUri }, compareMediaRefs);
    }
...
const HomeiiFavoritesFoundation = Object.freeze({
  ...
  } = {}, parseMediaReferenceFn = HomeiiMediaQueueFoundation.parseMediaReference) {
    const media = queueItem?.media_item || {};
    const playerUri = String(player?.attributes?.media_content_id || "").trim();
  ...
  favoriteEntryTitle(entry = {}) {
    return String(entry?.name || entry?.title || entry?.media_item?.name || "").trim();
  },
  normalizeComparableText(value = "") {
    return String(value || "").trim().toLowerCase();
  },
  favoriteRemoveArgsFromItem(item = {}, fallbackType = "track", parseMediaReferenceFn = HomeiiMediaQueueFoundation.parseMediaReference) {
    if (!item || typeof item !== "object") return null;
  ...
  findFavoriteEntryMatch(entry = {}, likedItems = [], {
    parseMediaReferenceFn = HomeiiMediaQueueFoundation.parseMediaReference,
  } = {}) {
    ...
    const targetTitle = HomeiiFavoritesFoundation.normalizeComparableText(HomeiiFavoritesFoundation.favoriteEntryTitle(entry));
    const targetArtist = HomeiiFavoritesFoundation.normalizeComparableText(HomeiiFavoritesFoundation.favoriteEntryArtist(entry));
    for (const item of likedItems) {
      ...
      if (targetLibraryId && candidateLibraryId && targetLibraryId === candidateLibraryId) return item;
      ...
      if (candidateUri && targetUri && candidateUri === targetUri) return item;
      ...
      const candidateTitle = HomeiiFavoritesFoundation.normalizeComparableText(HomeiiFavoritesFoundation.favoriteEntryTitle(item));
      const candidateArtist = HomeiiFavoritesFoundation.normalizeComparableText(HomeiiFavoritesFoundation.favoriteEntryArtist(item));
  ...
  matchFavoriteLibraryItem(entry = {}, likedItems = [], fallbackType = "", {
    parseMediaReferenceFn = HomeiiMediaQueueFoundation.parseMediaReference,
    favoriteRemoveArgsFromItemFn = HomeiiFavoritesFoundation.favoriteRemoveArgsFromItem,
    findFavoriteEntryMatchFn = HomeiiFavoritesFoundation.findFavoriteEntryMatch,
  } = {}) {
  ...
  buildOptimisticFavoriteEntry(entry = {}, liked = false) {
    return {
      uri: String(entry?.uri || "").trim(),
      ...
      artist: HomeiiFavoritesFoundation.favoriteEntryArtist(entry),
      album: entry?.album || entry?.media_item?.album?.name || "",
      image: entry?.image || entry?.media_item?.image || entry?.media_item?.album?.image || "",
      favorite: !!liked,
    };
  },
  applyOptimisticFavoriteCache(likedItems = [], nextEntry = {}, liked = false, {
    compareMediaRefsFn = HomeiiMediaQueueFoundation.mediaRefsEquivalent,
    matchFavoriteLibraryItemFn = HomeiiFavoritesFoundation.matchFavoriteLibraryItem,
  } = {}) {
  ...
  resolveCurrentMediaFavoriteState({
    ...
    if (!uri) return false;
    if (override?.uri === uri && Number(now || 0) - Number(override?.ts || 0) < 8000) {
...
const HomeiiPlayersFoundation = Object.freeze({
  ...
  isLikelyBrowserPlayer(player = null) {
    if (!player) return false;
    const attrs = player.attributes || {};
    const haystack = [
      player.entity_id,
  ...
  getBrowserPlayers(players = []) {
    return (Array.isArray(players) ? players : []).filter((player) => HomeiiPlayersFoundation.isLikelyBrowserPlayer(player));
  },
  getThisDevicePlayer(players = [], rememberedId = "") {
    const target = String(rememberedId || "").trim();
    if (!target) return null;
    return (Array.isArray(players) ? players : []).find((player) => player?.entity_id === target) || null;
  },
  resolvePinnedPlayerEntities(preferredIds = [], players = []) {
  ...
  playerByEntityId(entityId = "", players = [], hassStates = {}) {
  ...
  favoriteButtonEntityForPlayer({
    ...
    const tokens = [
      ...HomeiiPlayersFoundation.entityMatchTokens(player?.entity_id || ""),
      ...HomeiiPlayersFoundation.entityMatchTokens(player?.attributes?.friendly_name || ""),
    ].filter((token) => !HomeiiPlayersFoundation.genericPlayerTokens.has(token));
  ...
  favoriteButtonDeviceId(entityId = "", hassEntities = {}) {
    const target = String(entityId || "").trim();
    if (!target) return "";
    return String(hassEntities?.[target]?.device_id || "").trim();
  },
  announcementEligiblePlayers(players = []) {
    return (Array.isArray(players) ? players : [])
      .filter((player) => player?.entity_id)
      .filter((player) => !HomeiiPlayersFoundation.isLikelyBrowserPlayer(player));
  },
  mobileNavigableActivePlayers(players = [], pinnedEntityIds = [], isPlayerActiveFn = () => false) {
  ...
  playerGroupMemberIds(player = null) {
  ...
  isStaticGroupPlayer(player = null) {
    if (!player) return false;
    const attrs = player.attributes || {};
    const typeText = [
      player.entity_id,
  ...
  playerGroupCount(player = null) {
    if (HomeiiPlayersFoundation.isStaticGroupPlayer(player)) return 0;
    const count = HomeiiPlayersFoundation.playerGroupMemberIds(player).length;
    return count > 1 ? count : 0;
  },
  playerGroupMemberNames(player = null, players = []) {
    const ids = HomeiiPlayersFoundation.playerGroupMemberIds(player);
  ...
  groupAverageVolume(player = null, players = []) {
    const ids = HomeiiPlayersFoundation.playerGroupMemberIds(player);
    const byId = new Map((Array.isArray(players) ? players : []).map((entry) => [entry?.entity_id, entry]));
    const volumes = ids
  ...
  groupedPlayerIds(players = []) {
    const sourcePlayers = Array.isArray(players) ? players : [];
    const ids = new Set();
    const byId = new Map(sourcePlayers.map((player) => [player?.entity_id, player]));
    for (const player of sourcePlayers) {
      if (HomeiiPlayersFoundation.isLikelyBrowserPlayer(player)) continue;
      if (HomeiiPlayersFoundation.isStaticGroupPlayer(player)) continue;
      const members = HomeiiPlayersFoundation.playerGroupMemberIds(player);
      if (members.length > 1) {
        members
          ...
          .filter((entityId) => !HomeiiPlayersFoundation.isStaticGroupPlayer(byId.get(entityId)))
...
const HomeiiMediaPresentationFoundation = Object.freeze({
  ...
  sourceProviderMeta(value = "", { libraryLabel = "Library", radioLabel = "Radio" } = {}) {
  ...
  coerceLyricsText(value) {
    return HomeiiMediaPresentationFoundation.stripLyricsTimestamps(
      HomeiiMediaPresentationFoundation.coerceLyricsRawText(value),
    );
  },
  parseLrcLyrics(text = "") {
    const rows = [];
  ...
  extractCurrentLyricsRawText(queueItem = null) {
    ...
    for (const candidate of candidates) {
      const text = HomeiiMediaPresentationFoundation.coerceLyricsRawText(candidate);
      if (text) return text;
    }
    return "";
  },
  extractCurrentLyricsText(queueItem = null) {
    return HomeiiMediaPresentationFoundation.stripLyricsTimestamps(
      HomeiiMediaPresentationFoundation.extractCurrentLyricsRawText(queueItem),
    );
  },
  imageProxyUrl(path, provider = "", size = 300, maUrl = "") {
    if (!path) return null;
  ...
  imageUrl(value, size = 300, { maUrl = "", seen = new Set(), depth = 0 } = {}) {
    ...
    if (typeof value === "string") {
      ...
      return HomeiiMediaPresentationFoundation.imageProxyUrl(raw, "", size, maUrl);
    }
    ...
    if (Array.isArray(value)) {
      for (const entry of value) {
        const resolved = HomeiiMediaPresentationFoundation.imageUrl(entry, size, { maUrl, seen, depth: depth + 1 });
    ...
    if (rawPath) {
      return HomeiiMediaPresentationFoundation.imageProxyUrl(
        rawPath,
    ...
    for (const key of priorityKeys) {
      const resolved = HomeiiMediaPresentationFoundation.imageUrl(value[key], size, { maUrl, seen, depth: depth + 1 });
      if (resolved) return resolved;
    }
  ...
  normalizeMediaItem(item, maUrl = "") {
    if (!item || typeof item !== "object") return item;
    const imageUrl = HomeiiMediaPresentationFoundation.artUrl(item, maUrl);
...
const HomeiiMediaHistoryFoundation = Object.freeze({
  buildCurrentSourceBadgeMeta(
    ...
    {
      parseMediaReferenceFn = HomeiiMediaQueueFoundation.parseMediaReference,
      sourceProviderMetaFn = HomeiiMediaPresentationFoundation.sourceProviderMeta,
      qualityBadgeLabelFn = HomeiiMediaPresentationFoundation.qualityBadgeLabel,
  ...
  buildCurrentHistorySnapshot(
    ...
    } = {},
    {
      ...
      queueItemImageUrlFn,
      artUrlFn,
      buildCurrentSourceBadgeMetaFn = HomeiiMediaHistoryFoundation.buildCurrentSourceBadgeMeta,
    } = {},
  ) {
    const currentQueueItem = queueItem || null;
    const media = currentQueueItem?.media_item || currentQueueItem || {};
  ...
  applyRecentPlaybackSnapshot(snapshot = null, previousEntry = null, recentHistory = [], maxItems = 10) {
    if (!snapshot) {
      return {
  ...
  visibleRecentHistoryItems(currentEntry = null, recentHistory = [], limit = 5) {
...
function getMobileEditorTexts() {
  ...
  return {
    ...
    labels: {
      ...
      mobile_studio_shortcut: he ? "כפתור סטודיו" : "Studio shortcut",
      mobile_home_shortcut: he ? "כפתור בית" : "Home shortcut",
      mobile_home_shortcut_path: he ? "נתיב כפתור בית" : "Home shortcut path",
      ...
      pinned_player_entities: he ? "נגנים מקובעים" : "Pinned players",
      config_entry_id: "Config Entry ID",
      ma_url: he ? "כתובת Music Assistant" : "Music Assistant URL",
      ma_token: he ? "טוקן Music Assistant" : "Music Assistant token",
    ...
    helpers: {
      ...
      mobile_studio_shortcut: he ? "מפעיל או מכבה את כפתור הסטודיו בסרגל התחתון." : "Enable or disable the Studio button in the footer bar.",
      mobile_home_shortcut: he ? "מפעיל את כפתור הבית הצף." : "Enable the floating home shortcut button.",
      mobile_home_shortcut_path: he ? "למשל /lovelace/home או כל נתיב אחר ב-Home Assistant." : "For example /lovelace/home or any other Home Assistant path.",
    ...
    options: {
      ...
      mobile_background_motion_mode: [
        ...
        { value: "strong", label: he ? "חזק" : "Strong" },
        { value: "extreme", label: he ? "אקסטרים" : "Extreme" },
      ],
      mobile_swipe_mode: [
        ...
        schema: [
          {
            ...
            schema: [
              ...
              { name: "pinned_player_entities", selector: { entity: { multiple: true, filter: [{ domain: "media_player" }] } } },
            ],
          },
        ],
      },
...
class MABrowserCardEditor extends HTMLElement {
  ...
  set hass(hass) {
    ...
    if (previousLang !== nextLang || !this._editorRoot || !this._editorForm) {
      this._render();
      return;
    }
      ...
      <div class="editor-shell">
        <div class="editor-toolbar">
          <div class="path-actions">
            <button type="button" class="path-btn" id="editorUseCurrentPath"></button>
          </div>
          <div class="path-hint" id="editorPathHint"></div>
        </div>
  ...
  _isTabletStabilityMode() {
    try {
      ...
      const touchPoints = Number(window.navigator?.maxTouchPoints || 0);
      return /Android/i.test(ua) && width >= 900 && touchPoints > 0;
    } catch (_) {
      return false;
    }
  }
  ...
  _syncEditorLiveContext() {
    ...
    this.style.direction = he ? "rtl" : "ltr";
    this._refreshEditorShellClasses();
    if (this._editorUsePathBtn) {
      this._editorUsePathBtn.textContent = he ? "השתמש במסך הנוכחי עבור כפתור הבית" : "Use current view for Home button";
    }
    if (this._editorPathHint) {
      this._editorPathHint.textContent = `${he ? "נתיב נוכחי" : "Current path"}: ${this._currentUiPath()}`;
    }
    if (this._editorForm) {
  ...
  _render() {
    ...
    const schema = Array.isArray(formDef) ? formDef : (Array.isArray(formDef.schema) ? formDef.schema : []);
    ...
    if (this._editorForm) {
      ...
      if (this._editorLastConfigKey !== configKey) {
        this._editorForm.data = config;
        this._editorLastConfigKey = configKey;
      }
      this._editorForm.computeLabel = (item) => labels?.[item?.name] || item?.label || item?.name || "";
      this._editorForm.computeHelper = (item) => helpers?.[item?.name] || "";
    }
  }
}
...
class MABrowserCardMobile extends MABrowserCard {
  constructor() {
    ...
    try { this._state.mobileFontScale = Math.max(0.5, Math.min(1.5, Number(localStorage.getItem("ma_browser_card_mobile_font_scale") || 1) || 1)); } catch (_) {}
    ...
    try { this._state.mobileSleepTimerOrigin = localStorage.getItem("ma_browser_card_mobile_sleep_timer_origin") || ""; } catch (_) {}
    try { this._state.mobileLyricsSyncEnabled = JSON.parse(localStorage.getItem("ma_browser_card_mobile_lyrics_sync") ?? "true"); } catch (_) {}
    ...
    try { this._state.mobileStudioShortcutEnabled = JSON.parse(localStorage.getItem("ma_browser_card_mobile_studio_shortcut") ?? "true"); } catch (_) {}
    ...
    try { this._state.mobileHomeShortcutPath = localStorage.getItem("ma_browser_card_mobile_home_shortcut_path") || "/"; } catch (_) {}
    ...
    try {
      const rawPinned = JSON.parse(localStorage.getItem("ma_browser_card_mobile_pinned_players") || "[]");
    ...
    if (!Array.isArray(this._state.pinnedPlayerEntities) || !this._state.pinnedPlayerEntities.length) {
      try {
        const legacyPinned = localStorage.getItem("ma_browser_card_mobile_pinned_player") || "";
  ...
  _defaultMobileMediaLayout() {
    return HomeiiResponsiveFoundation.defaultMobileMediaLayout(this._layoutModeConfig());
  }
      ...
      schema: [
        ...
        },
        {
          ...
          schema: [
            ...
            { name: "mobile_background_motion_mode", selector: { select: { mode: "dropdown", options: t.options.mobile_background_motion_mode } } },
            { name: "mobile_custom_text_tone", selector: { select: { mode: "dropdown", options: t.options.mobile_custom_text_tone } } },
            { name: "mobile_font_scale", selector: { number: { min: 0.5, max: 1.5, step: 0.05, mode: "box" } } },
            { name: "mobile_footer_mode", selector: { select: { mode: "dropdown", options: t.options.mobile_footer_mode } } },
          ],
        },
        {
  ...
  _isCompactTileMode() {
    return HomeiiStateFoundation.isCompactTileMode(this._state);
  }
  ...
  _applyConfiguredMobileSettings() {
    ...
    if (this._usesVisualSettings()) {
      Object.assign(this._state, HomeiiMobileSettingsFoundation.normalizeVisualMobileState(cfg, {
  ...
  _layoutModeConfig() {
    return HomeiiResponsiveFoundation.resolveLayoutMode(this._config?.layout_mode, {
      rectWidth: Number(this.getBoundingClientRect?.().width || 0),
      hostWidth: Number(this.offsetWidth || 0),
  ...
  _sleepTimerCornerInnerHtml() {
    ...
    return `
      <div class="sleep-timer-menu" id="sleepTimerMenu"${menuOpen ? `` : ` hidden`}>
        ...
        <button class="sleep-timer-menu-btn danger" data-sleep-timer-clear>${this._esc(this._m("Cancel", "בטל"))}</button>
        <button class="sleep-timer-menu-btn ghost" data-sleep-timer-close>${this._esc(this._m("Close", "סגור"))}</button>
      </div>
      <button class="sleep-timer-chip active" id="sleepTimerChip" title="${this._esc(this._m("Sleep timer", "טיימר שינה"))}">
        ${this._iconSvg("timer")}
  ...
  _syncTabletAutoFitUi(hasUpNext = null) {
    ...
    const flags = HomeiiResponsiveFoundation.resolveTabletAutoFitFlags(this._layoutModeConfig(), {
      showNightRow,
  ...
  _setSleepTimerMinutes(minutes = 15, source = "general") {
    const amount = Math.max(1, Number(minutes) || 0);
    const player = this._getSelectedPlayer();
    if (!player?.entity_id) {
    ...
    this._state.mobileSleepTimerEndsAt = HomeiiNightFoundation.createSleepTimerTargetAt(amount, Date.now());
    this._state.mobileSleepTimerPlayer = player.entity_id;
    this._state.mobileSleepTimerOrigin = HomeiiNightFoundation.normalizeSleepTimerOrigin(source);
    this._state.mobileSleepTimerMenuOpen = false;
    this._persistMobileAppearance();
    this._syncNightModeUi();
    this._syncSleepTimerChip();
    this._toastSuccess(this._isHebrew() ? `טיימר שינה הופעל ל־${amount} דקות` : `Sleep timer set for ${amount} minutes`);
    return true;
  }

  _addSleepTimerMinutes(minutes = 15) {
    const amount = Math.max(1, Number(minutes) || 0);
    const player = this._getSelectedPlayer();
    this._state.mobileSleepTimerEndsAt = HomeiiNightFoundation.extendSleepTimerTargetAt(
      this._state.mobileSleepTimerEndsAt || 0,
      amount,
      Date.now(),
    );
    this._state.mobileSleepTimerPlayer = player?.entity_id || this._state.mobileSleepTimerPlayer || this._state.selectedPlayer || "";
    this._persistMobileAppearance();
    this._syncNightModeUi();
    this._syncSleepTimerChip();
    this._toastSuccess(this._isHebrew() ? `נוספו ${amount} דקות לטיימר` : `Added ${amount} minutes to the timer`);
  }
  ...
  _cycleSleepTimer(source = "general") {
    ...
    if (!currentRemaining) {
      ...
      this._state.mobileSleepTimerEndsAt = HomeiiNightFoundation.createSleepTimerTargetAt(steps[0], Date.now());
      this._state.mobileSleepTimerPlayer = player.entity_id;
      this._state.mobileSleepTimerOrigin = normalizedSource;
      this._state.mobileSleepTimerMenuOpen = false;
      this._persistMobileAppearance();
      this._syncNightModeUi();
      this._syncSleepTimerChip();
    ...
    const player = this._getSelectedPlayer();
    this._state.mobileSleepTimerEndsAt = HomeiiNightFoundation.createSleepTimerTargetAt(nextStep, Date.now());
    this._state.mobileSleepTimerPlayer = player?.entity_id || this._state.mobileSleepTimerPlayer || "";
    if (normalizedSource === "night") this._state.mobileSleepTimerOrigin = normalizedSource;
    this._state.mobileSleepTimerMenuOpen = false;
    this._persistMobileAppearance();
    this._syncNightModeUi();
    this._syncSleepTimerChip();
  ...
  _persistMobileAppearance() {
    ...
    try { localStorage.setItem("ma_browser_card_mobile_sleep_timer_origin", this._state.mobileSleepTimerOrigin || ""); } catch (_) {}
    try { localStorage.setItem("ma_browser_card_mobile_lyrics_sync", JSON.stringify(this._state.mobileLyricsSyncEnabled !== false)); } catch (_) {}
    ...
    try { localStorage.setItem("ma_browser_card_mobile_studio_shortcut", JSON.stringify(this._mobileStudioShortcutEnabled())); } catch (_) {}
    ...
    try { localStorage.setItem("ma_browser_card_mobile_home_shortcut_path", this._mobileHomeShortcutPath()); } catch (_) {}
    ...
    try { localStorage.setItem("ma_browser_card_mobile_pinned_players", JSON.stringify(this._pinnedPlayerPreferences())); } catch (_) {}
  ...
  _mobileMainBarItems() {
    const hidePlayers = this._pinnedPlayerCount() <= 1 && this._hasPinnedPlayer();
  ...
  _favoriteButtonEntityForPlayer(player = null) {
    return HomeiiPlayersFoundation.favoriteButtonEntityForPlayer({
      ...
      explicitEntity: this._config?.favorite_button_entity || "",
      fallbackEntity: "button.bathroom_favorite_current_song_2",
    });
  }
  ...
  _build() {
    ...
    const sleepTimerLabel = this._sleepTimerFooterLabel();
    ...
    if (controlRoomEnabled && studioShortcutEnabled) {
    ...
    const historyToggleFabHtml = !compactTileMode
      ...
      : ``;
    const sleepTimerCornerHtml = !compactTileMode ? `
    ...
    const centerHtml = `
      <div class="center">
        ...
        ${layoutMode === "tablet" ? `` : `<div class="hero-mobile-top">${playerFocusHtml}</div>`}
...
.player-sub { margin-top:1px; font-size:calc(11px * var(--v2-font-scale)); color:rgba(255,255,255,.72); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        ...
        .mobile-art-shell > .art-source-badges {
          ...
          inset-inline-start:clamp(4px, 1.1vw, 12px);
        }
        .art-aura {
          position:absolute;
          top:50%;
        ...
        .bottom { display:grid; width:100%; max-width:100%; min-width:0; gap:12px; align-content:end; }
        ...
        .side-btn.minor-btn { width:48px; height:48px; border-radius:16px; }
        .main-btn {
          width:118px;
          height:118px;
          border-radius:50%;
        ...
        .card.layout-tablet .volume-btn { width:64px; height:64px; border-radius:999px; }
        .card.layout-tablet .side-btn.minor-btn { width:54px; height:54px; border-radius:999px; }
        .card.layout-tablet .main-btn { width:110px; height:110px; }
        ...
        .lyrics-head-actions { display:flex; align-items:center; gap:10px; justify-self:end; }
        .lyrics-sync-btn {
          border:none;
          min-height:40px;
          padding:0 12px;
        ...
        @media (max-width:600px) {
          ...
          .art-stack-view,
          .art-stack-viewport,
          .art-stack-container { min-height:min(56vw, 268px); }
          .art-stack-slide { width:74%; }
          .art-stack-slide.center { width:78%; }
          .art-stack-slide.prev,
          .art-stack-slide.next { width:56%; }
          .mobile-art-shell { width:min(318px, calc(100% - 28px)); border-radius:0; padding:0; }
          ...
          .side-btn,.volume-btn { width:48px; height:48px; border-radius:18px; flex:0 0 auto; }
          .side-btn.minor-btn { width:40px; height:40px; border-radius:16px; flex:0 0 auto; }
          .main-btn { width:84px; height:84px; flex:0 0 auto; }
...
@media (max-width: 600px){.mobile-art-actions{gap:8px!important;padding:0!important;margin-top:0!important;}.mobile-art-actions.count-4{gap:8px!important;}.mobile-art-fab{width:38px;min-width:38px;height:38px;}}
...
.control-room-scene{position:relative;height:100%;min-height:100%;width:100%;padding:20px 22px 0;display:grid;overflow:hidden;}
...
.control-room-grid-wrap{min-height:0;height:100%;padding:24px 28px 0;overflow:hidden;display:block;}
...
.control-room-tile.selected{transform:translateY(-16px) scale(1.035);border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .54);box-shadow:0 36px 84px rgba(0,0,0,.42),0 0 0 1px rgba(var(--dynamic-accent-rgb,245 166 35) / .18),0 18px 40px rgba(var(--dynamic-accent-rgb,245 166 35) / .14);z-index:3;filter:saturate(1.06);}
.control-room-tile.primary{outline:1px solid rgba(255,255,255,.12);}
.control-room-grid:hover .control-room-tile:not(.selected){opacity:.82;transform:translateY(0) scale(.982);}
...
.control-room-tile-copy{display:grid;gap:8px;min-width:0;padding:10px 12px 8px;border-radius:22px;background:linear-gradient(180deg, rgba(8,11,18,.1), rgba(8,11,18,.26));backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);}
...
.control-room-float-pill,.control-room-primary-pill{display:inline-flex;align-items:center;min-height:26px;padding:0 10px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(9,12,18,.3);font-size:calc(10px * var(--v2-font-scale));font-weight:800;color:#fff;backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);}
...
.control-room-tile-track{font-size:calc(12px * var(--v2-font-scale));font-weight:700;color:rgba(255,255,255,.86);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 8px 18px rgba(0,0,0,.34);}
.control-room-tile-name{font-size:calc(18px * var(--v2-font-scale));font-weight:850;line-height:1.02;letter-spacing:-.024em;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 12px 28px rgba(0,0,0,.4);}
.control-room-tile-state{font-size:calc(11px * var(--v2-font-scale));font-weight:700;color:rgba(255,255,255,.7);}
...
.control-room-dock{position:relative;left:auto;bottom:auto;display:flex;align-items:center;justify-content:center;gap:18px;flex-wrap:nowrap;max-width:calc(100% - 8px);width:max-content;margin:0 auto;padding:16px 20px calc(18px + env(safe-area-inset-bottom, 0px));border-radius:30px 30px 0 0;border:1px solid rgba(255,255,255,.14);border-bottom:none;background:linear-gradient(180deg, rgba(17,22,31,.72), rgba(9,12,18,.56));backdrop-filter:blur(26px);-webkit-backdrop-filter:blur(26px);box-shadow:0 18px 48px rgba(0,0,0,.28);transform:none;align-self:end;z-index:2;}
...
.control-room-selection-pill{display:inline-flex;align-items:center;justify-content:center;min-width:56px;height:56px;padding:0 16px;border-radius:999px;background:rgba(var(--dynamic-accent-rgb,245 166 35) / .28);border:1px solid rgba(var(--dynamic-accent-rgb,245 166 35) / .32);font-size:calc(13px * var(--v2-font-scale));font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px;box-shadow:0 12px 28px rgba(0,0,0,.22);}
...
.control-room-dock-btn.library-pill{width:58px;min-width:58px;padding:0;}
.control-room-dock-label{font-size:calc(12px * var(--v2-font-scale));font-weight:800;white-space:nowrap;}
...
.card.layout-tablet.tablet-stable .media-entry,
...
@media (max-width: 1280px){
  ...
  .control-room-dock-section{padding:6px 8px;}
      ...
      <div class="card ${rtl ? "rtl" : ""} theme-${visualTheme} layout-${layoutMode}${compactMode ? " compact-mode" : ""}${compactTileMode ? " compact-collapsed" : compactMode ? " compact-expanded" : ""}${nightActive ? " night-mode" : ""}${showNightRow ? " night-mode-enabled" : ""}${tabletAutoFit ? " tablet-auto-fit" : ""}${tabletDenseUi ? " tablet-fit-dense" : ""}${showNightRow ? " tablet-fit-night" : ""}${showUpNextInline ? " tablet-fit-up-next" : ""}${this._tabletStabilityModeEnabled() ? " tablet-stable" : ""}">
        <div class="bg" id="mobileBg"></div><div class="shade"></div><div class="glow"></div>
        ${compactCollapseFabHtml}
        ${topSettingsFabHtml}
        ${homeShortcutFabHtml}
        ${historyToggleFabHtml}
        ${sleepTimerCornerHtml}
        <div class="stage">
    ...
    this.$("sleepTimerCorner")?.addEventListener("click", (e) => {
      ...
      if (addBtn) {
        this._addSleepTimerMinutes(Number(addBtn.dataset.sleepTimerAdd || 15));
    ...
    this.$("activePlayerChip")?.addEventListener("click", () => {
      ...
      if (this._hasPinnedPlayer() && this._pinnedPlayerCount() <= 1) {
        this._toast(this._m("Player is pinned from settings", "הנגן מקובע מתוך ההגדרות"));
        return;
      }
      ...
      <button class="menu-item action-tile tone-${this._esc(tone)}" data-sleep-timer-start="${this._esc(String(minutes))}">
        <span class="menu-item-main">
          ...
          <span style="min-width:0;flex:1;">
            ...
            <span class="menu-item-sub">${this._esc(this._m("Set sleep timer", "הפעל טיימר שינה"))}</span>
          </span>
        </span>
      </button>
    `;
    ...
    return `
      <div class="settings-shell">
        <div class="settings-group">
          <div class="settings-label">${this._esc(this._m("Sleep Timer", "טיימר שינה"))}</div>
          <div class="settings-hint">${this._esc(status)}</div>
        </div>
        <div class="action-grid">
          ${this._sleepTimerActionTile("+15", 15, "queue")}
          ${this._sleepTimerActionTile("+30", 30, "players")}
          ${this._sleepTimerActionTile("+60", 60, "announcement")}
          ${active ? `
            <button class="menu-item action-tile tone-stop" data-sleep-timer-cancel>
              <span class="menu-item-main">
                <span class="menu-item-ico">${this._iconSvg("close")}</span>
                <span style="min-width:0;flex:1;">
                  <span class="menu-item-title">${this._esc(this._m("Cancel Timer", "בטל טיימר"))}</span>
                  <span class="menu-item-sub">${this._esc(this._m("Stop the current countdown", "בטל את הספירה לאחור"))}</span>
                </span>
              </span>
            </button>
          ` : ``}
  ...
  _mainMenuHtml() {
    return `
      <div class="action-grid">
        ...
        ${this._navMenuItem("sleep_timer", this._iconSvg("timer"), this._m("Sleep Timer", "טיימר שינה"), this._m("Set playback to stop later", "הפעל טיימר לעצירת הניגון"), "announcement")}
  ...
  _settingsMenuHtml() {
    ...
    const visibleMainBarOptions = (pinnedPlayers.size > 0 && pinnedPlayers.size <= 1) ? mainBarOptions.filter(([value]) => value !== "players") : mainBarOptions;
      ...
      <div class="settings-shell">
        ...
        <div class="settings-group">
          ...
          <div class="settings-pills">
            ...
            ${this._settingsPill(this._m("Extreme", "אקסטרים"), "extreme", backgroundMotionMode, "data-setting-background-motion")}
          </div>
          <div class="settings-hint">${this._m("Adds motion to the background layers. Subtle keeps it calm, Strong is noticeable, and Extreme is the boldest animated look.", "מוסיף תנועה לשכבות הרקע. מצב עדין שומר על מראה רגוע, מצב חזק כבר מורגש, ואקסטרים נותן את התנועה הכי נועזת.")}</div>
          ...
          <div class="settings-label">${this._m("Home path", "נתיב הבית")}</div>
          <input class="media-sort-select settings-select" id="mobileHomeShortcutPathInput" type="text" value="${this._esc(this._mobileHomeShortcutPath())}" placeholder="/lovelace/home" aria-label="${this._esc(this._m("Home path", "נתיב הבית"))}">
          <div class="settings-hint">${this._m("Use any Home Assistant path. Example: /lovelace/home", "אפשר להשתמש בכל נתיב ב-Home Assistant. לדוגמה: /lovelace/home")}</div>
          <div class="settings-label">${this._m("Pinned players", "נגנים מקובעים")}</div>
          <div class="settings-check-grid">
          ...
          <div class="settings-hint">${this._m("Choose one or more players. The card stays inside this group and only switches between them.", "אפשר לבחור נגן אחד או יותר. הכרטיס נשאר בתוך הקבוצה הזו ורק עובר ביניהם.")}</div>
  ...
  async _renderMobileMenu() {
    ...
    const titles = {
      ...
      sleep_timer: this._m("Sleep Timer", "טיימר שינה"),
    ...
    else if (page === "sleep_timer") body.innerHTML = this._sleepTimerMenuHtml();
  ...
  async _handleMobileMenuClick(e) {
    ...
    const sleepTimerStartBtn = e.target.closest("[data-sleep-timer-start]");
    if (sleepTimerStartBtn?.dataset.sleepTimerStart) {
      e.preventDefault();
      e.stopPropagation();
      this._flashInteraction(sleepTimerStartBtn);
      this._setSleepTimerMinutes(Number(sleepTimerStartBtn.dataset.sleepTimerStart || 15), "general");
      await this._renderMobileMenu();
      return;
    }
    const sleepTimerCancelBtn = e.target.closest("[data-sleep-timer-cancel]");
    ...
    if (themeBtn?.dataset.settingTheme) {
      ...
      try { localStorage.setItem("ma_browser_card_theme", this._state.cardTheme); } catch (_) {}
      this._persistMobileAppearance();
      this._rebuildMobileUi({ reopenPage: "settings", reopenStudio: this._state.controlRoomOpen });
      return;
    }
    ...
    if (playerBtn) {
      const pinnedEntities = this._resolvedPinnedPlayerEntities();
      if (pinnedEntities.length && !pinnedEntities.includes(playerBtn.dataset.menuPlayer)) {
        this._toast(this._m("Player is pinned from settings", "הנגן מקובע מתוך ההגדרות"));
        return;
      }

...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _controlRoomPlayerTileHtml(player) {
    ...
    return `
      <article class="control-room-tile ${isSelected ? "selected" : ""} ${isPrimary ? "primary" : ""} ${playing ? "is-playing" : ""}" data-room-tile="${this._esc(player.entity_id)}" ${tileStyle}>
        ...
        <div class="control-room-tile-dock">
  ...
  _controlRoomHtml() {
    ...
    const anyPlaying = selectedIds.some((entityId) => this._playerByEntityId(entityId)?.state === "playing");
    return `
      <div class="control-room-scene ${primaryArt ? "has-art" : ""}" ${sceneStyle}>
        ...
        <div class="control-room-layout">
          ...
          <div class="control-room-dock">
            <span class="control-room-selection-pill">${this._esc(selectedIds.length > 1 ? `${selectedIds.length}` : (primary?.attributes?.friendly_name || "1"))}</span>
            <button class="control-room-dock-btn ${this._state.controlRoomPanel === "library" ? "active" : ""}" data-room-selection-action="library" title="${this._esc(this._m("Library", "ספריה"))}">
              ${this._iconSvg("search")}
            </button>
            ...
            <button class="control-room-dock-btn" data-room-selection-action="playpause" title="${this._esc(this._m("Play / Pause", "נגן / השהה"))}">
              ${this._iconSvg(anyPlaying ? "pause" : "play")}
            </button>
            <button class="control-room-dock-btn" data-room-selection-action="next" title="${this._esc(this._m("Next", "הבא"))}">
              ${this._iconSvg("next")}
            </button>
...
const HOMEII_CARD_VERSION = "4.7.1";
...
.control-room-backdrop{position:absolute;inset:0;display:flex;align-items:stretch;justify-content:center;padding:18px;opacity:0;pointer-events:none;transition:opacity .26s ease;z-index:28;background:rgba(6,10,18,.16);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);}
.control-room-backdrop.open{opacity:1;pointer-events:auto;}
.control-room-shell{position:relative;width:min(100%, 1480px);min-height:100%;max-height:100%;display:grid;grid-template-rows:auto minmax(0,1fr);border-radius:34px;overflow:hidden;border:1px solid rgba(255,255,255,.1);background:linear-gradient(180deg, rgba(8,11,18,.82), rgba(8,11,18,.92));box-shadow:0 30px 80px rgba(0,0,0,.3);}
.theme-light .control-room-shell{background:linear-gradient(180deg, rgba(248,250,253,.82), rgba(236,241,247,.9));border-color:rgba(30,42,64,.08);box-shadow:0 24px 60px rgba(23,37,61,.16);}
.control-room-head{position:absolute;inset-inline:0;inset-block-start:0;display:flex;justify-content:flex-end;padding:18px;z-index:4;pointer-events:none;}
...
.control-room-body-host{min-height:0;overflow:auto;padding:0;}
.control-room-scene{position:relative;min-height:100%;padding:34px 34px 28px;display:grid;}
...
.control-room-scene-bg{background:
  radial-gradient(circle at 18% 18%, rgba(var(--dynamic-accent-rgb,245 166 35) / .22), transparent 28%),
  radial-gradient(circle at 84% 16%, rgba(var(--dynamic-glow-rgb,255 175 92) / .16), transparent 24%),
  linear-gradient(180deg, rgba(255,255,255,.04), rgba(255,255,255,.01));}
.control-room-scene.has-art .control-room-scene-bg{background:
  linear-gradient(180deg, rgba(7,10,17,.14), rgba(7,10,17,.48)),
  var(--control-room-scene-art) center/cover no-repeat;}
.control-room-scene-glow{background:
  radial-gradient(circle at 24% 24%, rgba(var(--dynamic-accent-rgb,245 166 35) / .2), transparent 30%),
  radial-gradient(circle at 76% 68%, rgba(var(--dynamic-glow-rgb,255 175 92) / .14), transparent 28%);
  mix-blend-mode:screen;opacity:.8;}
.theme-light .control-room-scene-glow{opacity:.58;}
.control-room-layout{position:relative;z-index:1;min-height:100%;display:grid;grid-template-rows:minmax(0,1fr) auto auto;gap:18px;}
.control-room-grid-wrap{min-height:0;padding-top:28px;}
.control-room-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:18px;align-content:start;}
.control-room-tile{position:relative;min-height:320px;display:grid;grid-template-rows:minmax(0,1fr) auto auto;gap:10px;padding:18px;border-radius:30px;overflow:hidden;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);box-shadow:0 18px 40px rgba(0,0,0,.18);transition:transform .2s ease,border-color .2s ease,box-shadow .2s ease;}
.control-room-tile:hover{transform:translateY(-3px);}
.control-room-tile.selected{border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .48);box-shadow:0 20px 46px rgba(0,0,0,.24),0 0 0 1px rgba(var(--dynamic-accent-rgb,245 166 35) / .14);}
...
.control-room-tile[style*='--control-room-tile-art'] .control-room-tile-bg{background:var(--control-room-tile-art) center/cover no-repeat;}
.control-room-tile-shade{background:linear-gradient(180deg, rgba(10,13,20,.18) 0%, rgba(10,13,20,.18) 36%, rgba(10,13,20,.78) 100%);}
...
.control-room-tile-main,.control-room-tile-dock,.control-room-volume-row{position:relative;z-index:1;}
...
.control-room-tile-copy{display:grid;gap:8px;min-width:0;}
...
.control-room-tile-track{font-size:15px;font-weight:700;color:rgba(255,255,255,.92);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.control-room-tile-name{font-size:28px;font-weight:900;line-height:1;letter-spacing:-.03em;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
.control-room-tile-dock{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px;border-radius:20px;border:1px solid rgba(255,255,255,.12);background:rgba(9,12,18,.28);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);}
.theme-light .control-room-tile-dock{background:rgba(255,255,255,.58);border-color:rgba(28,42,68,.08);}
.control-room-chip-btn{width:42px;height:42px;padding:0;border-radius:15px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.08);color:#fff;display:grid;place-items:center;}
.control-room-chip-btn.active{background:rgba(var(--dynamic-accent-rgb,245 166 35) / .24);border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .28);}
.theme-light .control-room-chip-btn{background:rgba(245,248,252,.92);border-color:rgba(28,42,68,.08);color:#1f2a42;}
.control-room-chip-btn .ui-ic{width:18px;height:18px;}
.control-room-volume-row{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:10px;padding:0 4px;}
...
.control-room-tray{display:grid;gap:12px;align-self:end;max-width:min(960px, 100%);margin-inline:auto;padding:14px;border-radius:26px;border:1px solid rgba(255,255,255,.12);background:rgba(9,12,18,.32);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);box-shadow:0 18px 40px rgba(0,0,0,.18);}
...
.control-room-search{display:grid;grid-template-columns:20px minmax(0,1fr);align-items:center;gap:10px;padding:0 16px;}
...
.control-room-library-results{min-height:0;max-height:280px;overflow:auto;padding-inline-end:4px;}
...
.control-room-dock{display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap;max-width:max-content;margin-inline:auto;padding:12px 14px;border-radius:999px;border:1px solid rgba(255,255,255,.12);background:rgba(9,12,18,.3);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);box-shadow:0 14px 32px rgba(0,0,0,.18);}
.theme-light .control-room-dock{background:rgba(255,255,255,.62);border-color:rgba(28,42,68,.08);box-shadow:0 12px 24px rgba(28,42,68,.12);}
.control-room-selection-pill{display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:42px;padding:0 12px;border-radius:999px;background:rgba(var(--dynamic-accent-rgb,245 166 35) / .22);border:1px solid rgba(var(--dynamic-accent-rgb,245 166 35) / .26);font-size:12px;font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;}
.theme-light .control-room-selection-pill{color:#433006;}
.control-room-dock-btn{width:46px;height:46px;padding:0;border-radius:16px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.08);color:#fff;display:grid;place-items:center;}
.control-room-dock-btn.active{background:rgba(var(--dynamic-accent-rgb,245 166 35) / .24);border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .28);}
.theme-light .control-room-dock-btn{background:rgba(245,248,252,.94);border-color:rgba(28,42,68,.08);color:#1b2740;}
.control-room-dock-btn .ui-ic,.control-room-tray-btn .ui-ic{width:18px;height:18px;}
...
@media (max-width: 1280px){
  .control-room-grid{grid-template-columns:repeat(auto-fit,minmax(220px,1fr));}
}
    ...
    this.$("controlRoomBackdrop")?.addEventListener("click", async (e) => {
      ...
      if (dockBtn) {
        ...
        if (action === "library" || action === "transfer") {

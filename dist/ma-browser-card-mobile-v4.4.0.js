...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _syncMobileUpNextUi(item = null) {
    ...
    buttons.forEach((button) => {
      ...
      const subtitleEl = button.querySelector(".up-next-sub");
      if (!item) {
        button.hidden = true;
      ...
      const subtitle = [this._queueItemPrimaryArtist(item), item?.media_item?.album?.name || item?.album || ""].filter(Boolean).join(" · ");
      button.hidden = false;
      ...
      if (subtitleEl) subtitleEl.textContent = subtitle || this._m("Tap to skip to it", "לחץ כדי לעבור אליו");
    });
  }
...
const HOMEII_CARD_VERSION = "4.4.0";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _currentQuickMixEntry() {
    ...
    if (!uri || !this._supportsMusicAssistantRadioMode(mediaType)) return null;
    return {
      uri,
      media_type: mediaType,
      name: this._queueItemPrimaryTitle(currentQueueItem) || player?.attributes?.media_title || this._m("Quick mix", "מיקס מהיר"),
    };
  }

  async _startQuickMix() {
    const entry = this._currentQuickMixEntry();
    if (entry) {
            ...
            <div class="compact-main">
              <div class="compact-copy">
                ...
                <button class="up-next-inline compact-up-next" data-up-next-inline hidden>
                  <span class="up-next-kicker">${this._m("Up Next", "הבא בתור")}</span>
                  <span class="up-next-title"></span>
                  <span class="up-next-sub"></span>
                </button>
                ${nightQuickRowHtml}
              </div>
            </div>
      ...
      <style>
        ...
        * { box-sizing:border-box; }
        ...
        .up-next-inline {
          ...
          backdrop-filter:blur(16px);
          -webkit-backdrop-filter:blur(16px);
          transition:transform .16s ease, background-color .18s ease, border-color .18s ease, box-shadow .18s ease;
        }
        .up-next-inline:hover {
          transform:translateY(-1px);
          border-color:color-mix(in srgb, var(--ma-accent) 24%, rgba(255,255,255,.16));
        }
        ...
        .up-next-kicker {
          ...
          letter-spacing:.08em;
          text-transform:uppercase;
          color:color-mix(in srgb, var(--ma-accent) 66%, rgba(255,255,255,.72));
        }
        .up-next-title {
          font-size:12px;
          font-weight:800;

﻿﻿class MABrowserCard extends HTMLElement {
  constructor() {
    ...
    this._state = {
      ...
      mobilePlayerPickerMode: "players",
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        *::before,*::after { box-sizing:border-box; }
        ...
        .player-picker-backdrop.open { display:flex; }
        .player-picker-sheet {
          ...
          border:1px solid var(--ma-modal-border);
          background:
            linear-gradient(180deg, color-mix(in srgb, var(--ma-accent) 7%, transparent), transparent 20%),
            linear-gradient(180deg, color-mix(in srgb, var(--ma-modal-bg) 96%, transparent), color-mix(in srgb, var(--ma-modal-bg) 90%, black 10%));
        ...
        .theme-light .player-picker-sheet {
        ...
        .theme-light .player-picker-head {
          border-bottom-color:rgba(143,159,181,.16);
        }
        .player-picker-title-wrap { min-width:0; display:grid; gap:4px; }
        .player-picker-title { font-size:20px; font-weight:950; line-height:1.1; }
        .player-picker-subtitle { font-size:12px; color:var(--ma-text-3); font-weight:800; }
        .player-picker-body {
          overflow:auto;
          padding:18px;
        }
...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _playerPickerEls() {
    const mobileBackdrop = this.$("mobilePlayerPicker");
    if (mobileBackdrop) {
      return {
        mobile: true,
        backdrop: mobileBackdrop,
        title: this.$("mobilePlayerPickerTitle"),
        subtitle: this.$("mobilePlayerPickerSubtitle"),
        body: this.$("mobilePlayerPickerBody"),
        icon: null,
      };
    }
    return {
      mobile: false,
      backdrop: this.$("playerModal"),
      title: this.$("playerModalTitle"),
      subtitle: this.$("playerModalSubtitle"),
      body: this.$("playerModalBody"),
      icon: this.$("playerModalIcon"),
    };
  }

  _setPlayerModalHeader(mode = "players") {
    const picker = this._playerPickerEls();
  ...
  _renderPlayerModal() {
    ...
    if (picker.title) picker.title.textContent = this._t("Choose Player");
    this._setPlayerModalHeader("players");
    const body = picker.body;
    if (!body) return;
    const players = this._state.players || [];
    const showActiveOnly = this._state.mobilePlayerPickerMode === "players_active";
    ...
    body.innerHTML = `<div class="player-modal-grid">${showActiveOnly ? renderGroup(this._t("Playing"), activePlayers) : `${renderGroup(this._t("Playing"), activePlayers)}${renderGroup(this._t("Other players"), others)}`}</div>`;
  ...
  _openPlayerModal() {
    ...
    if (this.$("mobileMenuBody")) this.$("mobileMenuBody").innerHTML = "";
    if (!picker.mobile) this.shadowRoot?.appendChild(backdrop);
  ...
  _closePlayerModal() {
    ...
    this._state.playerModalOpen = false;
    this._state.modalMode = "players";
    if (!this._state.immersiveNowPlayingOpen && !this._state.mobileCompactExpanded) {
      this.classList.remove("top-layer-open");
    }
  }
  ...
  _openImmersiveNowPlaying() {
    ...
    this.classList.add("top-layer-open", "immersive-host-open");
  ...
  _closeImmersiveNowPlaying() {
    ...
    backdrop.innerHTML = "";
    this._state.immersiveNowPlayingOpen = false;
    if (this._state.returnToCompactAfterImmersive && this._mobileCompactModeEnabled()) {
      this._state.returnToCompactAfterImmersive = false;
      this._state.mobileCompactExpanded = false;
      this.classList.remove("top-layer-open", "immersive-host-open");
      this._build();
      this._init();
      return;
    }
    this._state.returnToCompactAfterImmersive = false;
    this.classList.remove("immersive-host-open");
    if (!this._state.playerModalOpen && !this._state.mobileCompactExpanded) this.classList.remove("top-layer-open");
  }
  ...
  _setView(view, btn = null) {
    ...
    const searchInp = this.$("searchInp");
    const searchClear = this.$("searchClear");
    if (searchWrap) searchWrap.style.display = isNowPlaying ? "none" : "";
    if (isNowPlaying) {
  ...
  async _playMedia(uri, mediaType = "album", enqueue = "play", options = {}) {
    ...
    try {
      ...
      if (!options.silent) this._toastMediaQueued(label, this._selectedPlayerName());
  ...
  async _applySpeakerGroup() {
    ...
    await this._hass.callService("media_player", "join", { entity_id: player.entity_id, group_members: groupMembers });
  ...
  async _transferQueueTo(targetPlayerEntityId) {
    ...
    try {
      ...
      this._selectPlayer(targetPlayerEntityId, true);
      this._toast(this._t("Queue transferred"));
...
const HOMEII_CARD_VERSION = "4.2.27";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _openCompactFullscreenPlayer() {
    this._state.returnToCompactAfterImmersive = false;
    this._closePlayerModal();
    this._closeMobileMenu();
    this._state.mobileCompactExpanded = true;
    this.classList.add("top-layer-open", "compact-expanded-host-open");
    this.classList.remove("immersive-host-open");
    this._build();
    this._init();
  }

  _openPlayerPickerForContext(page = "players") {
    this._closeMobileMenu();
    this._closePlayerModal();
    this._state.mobilePlayerPickerMode = page === "players_active" ? "players_active" : "players";
    this._loadPlayers();
    requestAnimationFrame(() => this._openPlayerModal());
  }

  _setCompactExpanded(expanded) {
    if (expanded && this._mobileCompactModeEnabled()) {
      this._openCompactFullscreenPlayer();
      return;
    }
    this._state.mobileCompactExpanded = false;
    this._closePlayerModal();
    this._closeMobileMenu();
    this.classList.remove("top-layer-open", "immersive-host-open", "compact-expanded-host-open");
    this._build();
    this._init();
  }
  ...
  _refreshMobileArtStack(force = false) {
    ...
    const previewArt = this._queueItemImageUrl(stack.current, 420)
      ...
      || playingArt;
    const art = previewArt || playingArt;
      ...
      <div class="center">
        ...
        <div class="art-stage">
          ...
          <div class="mobile-art-actions count-4" id="mobileArtActions">
            ...
            <button class="mobile-art-fab" id="mobileRandomBtn" title="${this._m("Surprise me", "תפתיע אותי")}">${this._iconSvg("wand")}</button>
          </div>
          </div>
        </div>
        <div class="hero-copy">
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { box-sizing:border-box; }
        ...
        :host(.top-layer-open.immersive-host-open) .card {
          width:100vw !important;
          max-width:none !important;
          height:100dvh !important;
        ...
        :host(.top-layer-open.compact-expanded-host-open) .card {
          width:100vw !important;
          max-width:none !important;
          height:100dvh !important;
          min-height:100dvh !important;
        ...
        .shade { background:linear-gradient(180deg, rgba(9,12,19,.18), rgba(9,12,19,.78) 38%, rgba(9,12,19,.98)); }
        .glow { background:radial-gradient(circle at 50% 76%, rgba(255,178,56,.2), transparent 30%); }
        .theme-light .bg {
...
.theme-light .player-mini-value{color:#435066!important;}
      ...
      <div class="card ${rtl ? "rtl" : ""} theme-${visualTheme} layout-${layoutMode}${compactMode ? " compact-mode" : ""}${compactTileMode ? " compact-collapsed" : compactMode ? " compact-expanded" : ""}">
        ...
        <div class="modal-backdrop" id="playerModal">
          <div class="modal">
            <div class="modal-header">
              <div class="modal-header-icon" id="playerModalIcon">🎵</div>
              <div class="modal-header-meta">
                <div class="modal-title" id="playerModalTitle">${this._m("Choose Player", "בחר נגן")}</div>
                <div class="modal-subtitle" id="playerModalSubtitle">—</div>
              </div>
              <button class="close-btn" id="playerModalClose">✕</button>
            </div>
        ...
        <div class="player-picker-backdrop" id="mobilePlayerPicker">
          <div class="player-picker-sheet">
            <div class="player-picker-head">
              <div class="modal-header-icon">${this._iconSvg("speaker")}</div>
              <div class="player-picker-title-wrap">
                <div class="player-picker-title" id="mobilePlayerPickerTitle">${this._m("Choose Player", "בחר נגן")}</div>
                <div class="player-picker-subtitle" id="mobilePlayerPickerSubtitle">—</div>
              </div>
              <button class="close-btn" id="mobilePlayerPickerClose">✕</button>
            </div>
            <div class="player-picker-body" id="mobilePlayerPickerBody"></div>
          </div>
        </div>
    ...
    this.$("playerModal")?.addEventListener("click", (e) => {
      if (e.target === this.$("playerModal")) this._closePlayerModal();
    });
    this.$("mobilePlayerPickerClose")?.addEventListener("click", () => this._closePlayerModal());
    this.$("mobilePlayerPicker")?.addEventListener("click", (e) => {
      if (e.target === this.$("mobilePlayerPicker")) this._closePlayerModal();
    });
    ...
    this.shadowRoot.querySelectorAll("[data-mainbar-action]").forEach((btn) => btn.addEventListener("click", () => {
      ...
      else if (action === "players") this._openPlayerPickerForContext("players");
    ...
    this.$("activePlayerChip")?.addEventListener("click", () => {
      ...
      this._openPlayerPickerForContext("players");
    });
    ...
    this.$("activePlayersBubble")?.addEventListener("click", () => this._openPlayerPickerForContext("players_active"));
    ...
    this.$("mobileMenuTitle")?.addEventListener("click", (e) => {
      if (e.currentTarget?.dataset?.menuTitleAction === "players") this._openPlayerPickerForContext("players");
    });
  ...
  _syncNowPlayingUI() {
    const player = this._getSelectedPlayer();
    const compactTileMode = this._isCompactTileMode();
    const renderCompactTile = ({ title = "", subtitle = "", art = "", icon = "music_note", duration = 0, position = 0, emptyAction = "" }) => {
    ...
    const bg = this.$("mobileBg");
    if (bg) {
      const overlay = this._effectiveTheme() === "light"
        ? `radial-gradient(circle at 18% 20%, rgba(255,187,88,.28), transparent 30%), radial-gradient(circle at 84% 16%, rgba(255,150,108,.16), transparent 22%), linear-gradient(180deg, rgba(255,255,255,.24), rgba(236,242,248,.72), rgba(228,235,244,.92))`
        : `radial-gradient(circle at 18% 20%, rgba(255,181,64,.24), transparent 32%), radial-gradient(circle at 82% 16%, rgba(255,128,76,.12), transparent 20%), linear-gradient(180deg, rgba(9,12,19,.26), rgba(9,12,19,.82), rgba(9,12,19,.98))`;
  ...
  _openMobileMenu(page = "main") {
    ...
    if ((page === "players" || page === "players_active") && this._hasPinnedPlayer()) {
      this._toast(this._m("Player is pinned from settings", "הנגן מקובע מתוך ההגדרות"));
      return;
    }
    if (page === "players" || page === "players_active") {
      this._openPlayerPickerForContext(page);
      return;
    }
  ...
  async _handleMobileMenuClick(e) {
    ...
    if (nav) {
      ...
      if (nav.dataset.menuNav === "players" || nav.dataset.menuNav === "players_active") {
        this._openPlayerPickerForContext(nav.dataset.menuNav);
        return;
      }

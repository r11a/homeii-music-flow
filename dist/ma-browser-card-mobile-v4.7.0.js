...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _controlRoomSelectedPlayerIds() {
    ...
    if (!selected.length && preferred && validIds.has(preferred)) selected = [preferred];
    if (!selected.length && players[0]?.entity_id) selected = [players[0].entity_id];
    this._state.controlRoomSelectedPlayers = selected;
    return selected;
  }

  _controlRoomPrimaryPlayerId() {
    return this._controlRoomSelectedPlayerIds()[0] || "";
  }
  ...
  _controlRoomPlayerTileHtml(player) {
    ...
    return `
      <article class="control-room-tile ${isSelected ? "selected" : ""} ${isPrimary ? "primary" : ""} ${playing ? "is-playing" : ""}" data-room-tile="${this._esc(player.entity_id)}">
        <button class="control-room-tile-main" data-room-primary="${this._esc(player.entity_id)}" title="${this._esc(name)}">
          <span class="control-room-tile-art">
            ${art ? `<img src="${this._esc(art)}" alt="">` : this._iconSvg("speaker")}
            ${groupCount ? `<span class="control-room-group-badge">${this._esc(String(groupCount))}</span>` : ``}
          </span>
          <span class="control-room-tile-copy">
            <span class="control-room-tile-topline">
        ...
        <div class="control-room-tile-controls">
          <button class="control-room-chip-btn ${isSelected ? "active" : ""}" data-room-select="${this._esc(player.entity_id)}" title="${this._esc(this._m("Select player", "בחר נגן"))}">
            ${this._iconSvg(isSelected ? "check" : "grid")}
          </button>
          <button class="control-room-chip-btn" data-room-toggle-play="${this._esc(player.entity_id)}" title="${this._esc(this._m("Play or pause", "נגן או השהה"))}">
            ${this._iconSvg(playing ? "pause" : "play")}
          </button>
          <button class="control-room-chip-btn" data-room-next="${this._esc(player.entity_id)}" title="${this._esc(this._m("Next track", "השיר הבא"))}">
            ${this._iconSvg("next")}
          </button>
          <button class="control-room-chip-btn ${this._isMuted(player) ? "active" : ""}" data-room-mute="${this._esc(player.entity_id)}" title="${this._esc(this._m("Mute", "השתק"))}">
            ${this._iconSvg(this._volumeIconName(player))}
          </button>
        </div>
        <label class="control-room-volume-row">
          <span class="control-room-volume-ico">${this._iconSvg("speaker")}</span>
  ...
  _controlRoomLibraryResultsHtml() {
    ...
    if (loading) return `<div class="control-room-empty">${this._esc(this._m("Searching library...", "מחפש בספריה..."))}</div>`;
    if (!query) return `<div class="control-room-empty">${this._esc(this._m("Search for a playlist, album, track or radio station.", "חפש פלייליסט, אלבום, שיר או תחנת רדיו."))}</div>`;
    if (!results.length) return `<div class="control-room-empty">${this._esc(this._m("No media found for this search.", "לא נמצאה מדיה לחיפוש הזה."))}</div>`;
    return results.map((entry, index) => `
      <button class="control-room-library-item" data-room-library-play="${this._esc(String(index))}">
        <span class="control-room-library-art">${entry.image ? `<img src="${this._esc(entry.image)}" alt="">` : this._iconSvg(this._controlRoomMediaTypeIcon(entry.media_type))}</span>
        <span class="control-room-library-copy">
          <span class="control-room-library-title">${this._esc(entry.name || this._m("Media", "מדיה"))}</span>
          <span class="control-room-library-sub">${this._esc(entry.subtitle || entry.media_type || "")}</span>
        </span>
        <span class="control-room-library-action">${this._iconSvg("play")}</span>
      </button>
    `).join("");
  }
  ...
  _controlRoomHtml() {
    ...
    const selectionTitle = selectedIds.length > 1
      ? this._m(`${selectedIds.length} players selected`, `נבחרו ${selectedIds.length} נגנים`)
      : this._m("Single player selected", "נגן אחד נבחר");
    return `
      <div class="control-room-layout">
        <section class="control-room-hero">
          <div class="control-room-hero-copy">
            <span class="control-room-kicker">${this._esc(this._m("Large screen overview", "תצוגת שליטה למסך גדול"))}</span>
            <h3>${this._esc(this._m("Control Room", "חדר בקרה"))}</h3>
            <p>${this._esc(this._m("Manage every player, build groups, transfer a queue and launch media from one central workspace.", "נהל כל נגן, חבר קבוצה, העבר תור והפעל מדיה מתוך חלון מרכזי אחד."))}</p>
          </div>
          <div class="control-room-summary-card">
            <div class="control-room-summary-label">${this._esc(selectionTitle)}</div>
            <div class="control-room-summary-name">${this._esc(primary?.attributes?.friendly_name || this._m("Select a player", "בחר נגן"))}</div>
            <div class="control-room-summary-sub">${this._esc(primary?.attributes?.media_title || primary?.attributes?.media_artist || this._playerStateLabel(primary) || "—")}</div>
          </div>
        </section>
        <div class="control-room-main">
          <section class="control-room-grid-panel">
            <div class="control-room-section-head">
              <div>
                <div class="control-room-section-title">${this._esc(this._m("Players", "נגנים"))}</div>
                <div class="control-room-section-sub">${this._esc(this._m("Choose one primary player or add more for combined actions.", "בחר נגן ראשי אחד או הוסף נגנים לפעולות משותפות."))}</div>
              </div>
            </div>
          ...
          <aside class="control-room-side">
            <section class="control-room-panel-card">
              <div class="control-room-panel-title">${this._esc(this._m("Queue transfer", "העברת תור"))}</div>
              <div class="control-room-transfer-grid">
                <label class="control-room-field">
                  <span>${this._esc(this._m("Source", "מקור"))}</span>
                ...
                <label class="control-room-field">
                  <span>${this._esc(this._m("Target", "יעד"))}</span>
              ...
              <button class="control-room-action-btn" data-room-transfer>${this._iconSvg("queue")}<span>${this._esc(this._m("Transfer queue", "העבר תור"))}</span></button>
            </section>
            <section class="control-room-panel-card">
              <div class="control-room-panel-title">${this._esc(this._m("Library picker", "בחירת מדיה מהספריה"))}</div>
              <label class="control-room-search">
                ${this._iconSvg("search")}
                <input id="controlRoomLibraryInput" type="search" placeholder="${this._esc(this._m("Search the library...", "חפש בספריה..."))}" value="${this._esc(this._state.controlRoomLibraryQuery || "")}">
              </label>
        ...
        <div class="control-room-dock">
          <button class="control-room-dock-btn" data-room-selection-action="playpause">${this._iconSvg("play")}${this._esc(this._m("Play / Pause", "נגן / השהה"))}</button>
          <button class="control-room-dock-btn" data-room-selection-action="next">${this._iconSvg("next")}${this._esc(this._m("Next", "הבא"))}</button>
          <button class="control-room-dock-btn" data-room-selection-action="group">${this._iconSvg("speaker")}${this._esc(this._m("Group selected", "חבר קבוצה"))}</button>
          <button class="control-room-dock-btn" data-room-selection-action="ungroup">${this._iconSvg("close")}${this._esc(this._m("Ungroup", "נתק קבוצה"))}</button>
        </div>
      </div>
    `;
  }
  ...
  _syncControlRoomUi() {
    ...
    if (!host) return;
    host.innerHTML = this._controlRoomHtml();
    const input = this.$("controlRoomLibraryInput");
    if (input) input.value = this._state.controlRoomLibraryQuery || "";
  }
...
const HOMEII_CARD_VERSION = "4.7.0";
      ...
      <div class="control-room-backdrop" id="controlRoomBackdrop">
        <div class="control-room-shell">
          <div class="control-room-head">
            <div>
              <div class="control-room-title">${this._esc(this._m("Control Room", "חדר בקרה"))}</div>
              <div class="control-room-subtitle">${this._esc(this._m("Unified multi-player control for large screens", "שליטה מרכזית בכמה נגנים למסכים גדולים"))}</div>
            </div>
...
.control-room-backdrop{position:absolute;inset:0;display:flex;align-items:stretch;justify-content:center;padding:22px;opacity:0;pointer-events:none;transition:opacity .26s ease;z-index:28;background:rgba(6,10,18,.18);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);}
.control-room-backdrop.open{opacity:1;pointer-events:auto;}
.control-room-shell{width:min(100%, 1440px);min-height:100%;max-height:100%;display:grid;grid-template-rows:auto minmax(0,1fr);border-radius:34px;background:linear-gradient(180deg, rgba(13,16,24,.74), rgba(11,14,22,.88));border:1px solid rgba(255,255,255,.12);box-shadow:0 26px 64px rgba(0,0,0,.28);overflow:hidden;}
.theme-light .control-room-shell{background:linear-gradient(180deg, rgba(255,255,255,.82), rgba(239,244,249,.92));border-color:rgba(30,42,64,.1);box-shadow:0 24px 56px rgba(20,34,58,.16);}
.control-room-head{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:24px 28px 20px;border-bottom:1px solid rgba(255,255,255,.08);}
.theme-light .control-room-head{border-color:rgba(33,48,74,.08);}
.control-room-title{font-size:clamp(24px, 2vw, 30px);font-weight:800;letter-spacing:-.03em;color:#fff;}
.theme-light .control-room-title{color:#152237;}
.control-room-subtitle{font-size:13px;color:rgba(255,255,255,.7);}
.theme-light .control-room-subtitle{color:#607087;}
.control-room-close{width:46px;height:46px;border-radius:16px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.08);color:#fff;display:grid;place-items:center;}
.theme-light .control-room-close{border-color:rgba(26,39,61,.12);background:rgba(255,255,255,.72);color:#1c2940;}
.control-room-close .ui-ic{width:19px;height:19px;}
.control-room-body-host{min-height:0;overflow:auto;padding:0 28px 28px;}
.control-room-layout{display:grid;grid-template-rows:auto minmax(0,1fr) auto;gap:22px;min-height:100%;}
.control-room-hero{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(280px, .8fr);gap:20px;padding-top:22px;}
.control-room-kicker{display:inline-flex;align-items:center;min-height:28px;padding:0 12px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);font-size:12px;font-weight:700;color:rgba(255,255,255,.76);}
.theme-light .control-room-kicker{background:rgba(255,255,255,.72);border-color:rgba(28,42,68,.1);color:#647389;}
.control-room-hero-copy h3{margin:10px 0 10px;font-size:clamp(30px, 2.5vw, 44px);line-height:1;letter-spacing:-.04em;color:#fff;}
.theme-light .control-room-hero-copy h3{color:#142135;}
.control-room-hero-copy p{max-width:720px;margin:0;font-size:15px;line-height:1.6;color:rgba(255,255,255,.72);}
.theme-light .control-room-hero-copy p{color:#617086;}
.control-room-summary-card,.control-room-panel-card,.control-room-grid-panel{border-radius:28px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.06);box-shadow:0 18px 42px rgba(0,0,0,.16);}
.theme-light .control-room-summary-card,.theme-light .control-room-panel-card,.theme-light .control-room-grid-panel{background:rgba(255,255,255,.78);border-color:rgba(28,42,68,.09);box-shadow:0 16px 34px rgba(28,42,68,.12);}
.control-room-summary-card{padding:22px;display:grid;align-content:center;gap:8px;}
.control-room-summary-label{font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.58);}
.theme-light .control-room-summary-label{color:#728198;}
.control-room-summary-name{font-size:24px;font-weight:800;color:#fff;line-height:1.1;}
.theme-light .control-room-summary-name{color:#17253a;}
.control-room-summary-sub{font-size:14px;color:rgba(255,255,255,.68);}
.theme-light .control-room-summary-sub{color:#5f6f86;}
.control-room-main{min-height:0;display:grid;grid-template-columns:minmax(0,1.45fr) minmax(320px,.72fr);gap:22px;}
.control-room-grid-panel{padding:22px;display:grid;grid-template-rows:auto minmax(0,1fr);gap:18px;}
.control-room-section-head{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;}
.control-room-section-title,.control-room-panel-title{font-size:18px;font-weight:800;color:#fff;}
.theme-light .control-room-section-title,.theme-light .control-room-panel-title{color:#18253b;}
.control-room-section-sub{margin-top:6px;font-size:13px;color:rgba(255,255,255,.62);}
.theme-light .control-room-section-sub{color:#69798f;}
.control-room-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px;align-content:start;}
.control-room-tile{display:grid;gap:14px;padding:16px;border-radius:24px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);transition:transform .18s ease,border-color .18s ease,background .18s ease,box-shadow .18s ease;}
.control-room-tile.selected{border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .42);background:rgba(var(--dynamic-surface-rgb,245 166 35) / .12);box-shadow:0 14px 30px rgba(0,0,0,.18);}
.control-room-tile.primary{outline:1px solid rgba(255,255,255,.1);}
.theme-light .control-room-tile{background:rgba(255,255,255,.72);border-color:rgba(31,45,68,.08);}
.theme-light .control-room-tile.selected{background:rgba(var(--dynamic-surface-rgb,245 166 35) / .14);border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .34);}
.control-room-tile-main{display:grid;grid-template-columns:72px minmax(0,1fr);gap:14px;align-items:center;background:none;border:none;color:inherit;text-align:inherit;padding:0;}
.control-room-tile-art{position:relative;width:72px;height:72px;border-radius:20px;overflow:hidden;background:rgba(255,255,255,.08);display:grid;place-items:center;color:rgba(255,255,255,.75);}
.theme-light .control-room-tile-art{background:rgba(240,244,248,.96);color:#4d5d74;}
.control-room-tile-art img{width:100%;height:100%;object-fit:cover;display:block;}
.control-room-group-badge{position:absolute;inset-inline-end:6px;inset-block-start:6px;min-width:24px;height:24px;padding:0 7px;border-radius:999px;background:rgba(8,10,16,.72);border:1px solid rgba(255,255,255,.14);font-size:11px;font-weight:800;color:#fff;display:grid;place-items:center;}
.control-room-tile-copy{display:grid;gap:6px;min-width:0;}
.control-room-tile-topline{display:flex;align-items:center;gap:8px;min-width:0;}
.control-room-tile-name{font-size:18px;font-weight:800;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.theme-light .control-room-tile-name{color:#18253b;}
.control-room-primary-pill{display:inline-flex;align-items:center;min-height:24px;padding:0 8px;border-radius:999px;background:rgba(var(--dynamic-accent-rgb,245 166 35) / .18);border:1px solid rgba(var(--dynamic-accent-rgb,245 166 35) / .24);font-size:11px;font-weight:700;color:#fff;}
.theme-light .control-room-primary-pill{color:#3e2c07;}
.control-room-tile-track,.control-room-tile-state{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.control-room-tile-track{font-size:14px;color:rgba(255,255,255,.8);}
.control-room-tile-state{font-size:12px;color:rgba(255,255,255,.56);}
.theme-light .control-room-tile-track{color:#4e5e74;}
.theme-light .control-room-tile-state{color:#7a879b;}
.control-room-tile-controls{display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
.control-room-chip-btn{min-width:42px;height:42px;padding:0 12px;border-radius:14px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.08);color:#fff;display:grid;place-items:center;}
.control-room-chip-btn.active{background:rgba(var(--dynamic-accent-rgb,245 166 35) / .22);border-color:rgba(var(--dynamic-accent-rgb,245 166 35) / .34);}
.theme-light .control-room-chip-btn{border-color:rgba(31,45,68,.09);background:rgba(245,248,251,.92);color:#1d2940;}
.control-room-chip-btn .ui-ic{width:18px;height:18px;}
.control-room-volume-row{display:grid;grid-template-columns:24px minmax(0,1fr) auto;align-items:center;gap:10px;}
.control-room-volume-ico{display:grid;place-items:center;color:rgba(255,255,255,.56);}
.theme-light .control-room-volume-ico{color:#6e7c90;}
.control-room-volume{width:100%;}
.control-room-volume-value{font-size:12px;font-weight:700;color:rgba(255,255,255,.72);}
.theme-light .control-room-volume-value{color:#506178;}
.control-room-side{display:grid;grid-template-rows:auto minmax(0,1fr);gap:18px;min-height:0;}
.control-room-panel-card{padding:20px;display:grid;gap:14px;}
.control-room-transfer-grid{display:grid;grid-template-columns:1fr;gap:12px;}
.control-room-field{display:grid;gap:7px;font-size:12px;font-weight:700;color:rgba(255,255,255,.64);}
.theme-light .control-room-field{color:#73829a;}
.control-room-field select,.control-room-search{min-height:48px;border-radius:16px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.08);color:#fff;}
.theme-light .control-room-field select,.theme-light .control-room-search{border-color:rgba(28,42,68,.1);background:rgba(245,248,252,.96);color:#152237;}
.control-room-field select{padding:0 14px;font:inherit;outline:none;}
.control-room-search{display:grid;grid-template-columns:22px minmax(0,1fr);align-items:center;gap:10px;padding:0 14px;}
...
.control-room-action-btn,.control-room-dock-btn{min-height:48px;border-radius:16px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.08);color:#fff;display:inline-flex;align-items:center;justify-content:center;gap:10px;padding:0 16px;font-size:14px;font-weight:800;}
.theme-light .control-room-action-btn,.theme-light .control-room-dock-btn{border-color:rgba(29,42,66,.1);background:rgba(255,255,255,.82);color:#152238;}
.control-room-library-results{min-height:0;max-height:360px;overflow:auto;display:grid;gap:10px;padding-inline-end:4px;}
.control-room-library-item{display:grid;grid-template-columns:42px minmax(0,1fr) 18px;align-items:center;gap:12px;padding:10px 12px;border-radius:16px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.04);color:inherit;text-align:inherit;}
.theme-light .control-room-library-item{background:rgba(247,249,252,.96);border-color:rgba(30,44,68,.08);}
.control-room-library-art{width:42px;height:42px;border-radius:12px;overflow:hidden;background:rgba(255,255,255,.08);display:grid;place-items:center;color:rgba(255,255,255,.75);}
.theme-light .control-room-library-art{background:rgba(235,240,245,.96);color:#54657c;}
.control-room-library-art img{width:100%;height:100%;object-fit:cover;display:block;}
.control-room-library-copy{display:grid;gap:4px;min-width:0;}
.control-room-library-title,.control-room-library-sub{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.control-room-library-title{font-size:14px;font-weight:700;color:#fff;}
.control-room-library-sub{font-size:12px;color:rgba(255,255,255,.56);}
.theme-light .control-room-library-title{color:#17253a;}
.theme-light .control-room-library-sub{color:#708099;}
.control-room-library-action .ui-ic{width:16px;height:16px;}
.control-room-empty{min-height:96px;display:grid;place-items:center;padding:20px 12px;border-radius:18px;border:1px dashed rgba(255,255,255,.12);color:rgba(255,255,255,.58);text-align:center;font-size:13px;}
.theme-light .control-room-empty{border-color:rgba(29,43,67,.12);color:#708097;}
.control-room-dock{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding-bottom:4px;}
.control-room-dock-btn .ui-ic{width:17px;height:17px;}
...
@media (max-width: 1180px){
  .control-room-hero,.control-room-main{grid-template-columns:1fr;}
  .control-room-library-results{max-height:260px;}
}
    ...
    this.$("controlRoomBackdrop")?.addEventListener("click", async (e) => {
      ...
      if (selectBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(selectBtn);
      ...
      if (primaryBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(primaryBtn);
      ...
      if (playBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(playBtn);
      ...
      if (nextBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(nextBtn);
      ...
      if (muteBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(muteBtn);
      ...
      if (transferBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(transferBtn);
      ...
      if (libraryPlayBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(libraryPlayBtn);
        const entry = (this._state.controlRoomLibraryResults || [])[Number(libraryPlayBtn.dataset.roomLibraryPlay)];
        const targetIds = this._controlRoomSelectedPlayerIds();
        if (!entry?.uri || !targetIds.length) return;
        await this._playMediaOnPlayers(targetIds, entry.uri, entry.media_type || "album", "play", { label: entry.name || "" });
      ...
      if (dockBtn) {
        e.preventDefault();
        e.stopPropagation();
        this._flashInteraction(dockBtn);
        ...
        if (action === "group") {
          ...
          await this._applySpeakerGroupFor(primaryId, members);

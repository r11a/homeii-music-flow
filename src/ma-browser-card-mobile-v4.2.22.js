...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _openPlayerModal() {
    ...
    this.shadowRoot?.appendChild(this.$("playerModal"));
  ...
  _closePlayerModal() {
    ...
    this._state.playerModalOpen = false;
    this._state.modalMode = "players";
    if (!this._state.immersiveNowPlayingOpen) this.classList.remove("top-layer-open");
  }
  ...
  _closeImmersiveNowPlaying() {
    ...
    this._state.returnToCompactAfterImmersive = false;
    if (!this._state.playerModalOpen) this.classList.remove("top-layer-open");
  }
...
const HOMEII_CARD_VERSION = "4.2.22";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _openCompactFullscreenPlayer() {
    ...
    this._build();
    this._init();
    requestAnimationFrame(() => requestAnimationFrame(() => this._openImmersiveNowPlaying()));
  }

  _openPlayerPickerForContext(page = "players") {
    if (this._layoutModeConfig() === "tablet" || this._isCompactTileMode()) {
      this._closeMobileMenu();
      this._loadPlayers();
      this._build();
      this._init();

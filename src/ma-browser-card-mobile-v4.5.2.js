...
const HOMEII_CARD_VERSION = "4.5.2";
  ...
  _applyBackgroundMotionStyles() {
    ...
    const vars = mode === "strong"
      ? {
          ...
          "--bg-motion-shift": "34px",
          "--bg-motion-scale": "1.14",
      ...
      : {
          ...
          "--bg-motion-shift": "22px",
          "--bg-motion-scale": "1.11",
          "--bg-motion-duration": "30s",
          "--glow-motion-duration": "22s",
          "--aura-motion-duration": "26s",
        };
      ...
      <style>
        ...
        * { box-sizing:border-box; }
        ...
        @keyframes backgroundFloat {
          0% { transform:translate3d(0, 0, 0) scale(var(--bg-motion-scale, 1.11)); }
          25% { transform:translate3d(calc(var(--bg-motion-shift, 22px) * .58), calc(var(--bg-motion-shift, 22px) * -.4), 0) scale(calc(var(--bg-motion-scale, 1.11) + .018)); }
          55% { transform:translate3d(calc(var(--bg-motion-shift, 22px) * -.72), calc(var(--bg-motion-shift, 22px) * .46), 0) scale(calc(var(--bg-motion-scale, 1.11) + .038)); }
          80% { transform:translate3d(calc(var(--bg-motion-shift, 22px) * .22), calc(var(--bg-motion-shift, 22px) * .18), 0) scale(calc(var(--bg-motion-scale, 1.11) + .012)); }
          100% { transform:translate3d(0, 0, 0) scale(var(--bg-motion-scale, 1.11)); }
        }
        @keyframes glowDrift {
          0% { transform:translate3d(0, 0, 0) scale(1); opacity:.9; }
          50% { transform:translate3d(calc(var(--bg-motion-shift, 22px) * -.5), calc(var(--bg-motion-shift, 22px) * .34), 0) scale(calc(1 + (.055 * var(--bg-motion-strength, 1)))); opacity:1; }
          100% { transform:translate3d(0, 0, 0) scale(1); opacity:.9; }
        }
        @keyframes auraDrift {
          0% { transform:translate3d(0, 0, 0) scale(1.08); }
          50% { transform:translate3d(calc(var(--bg-motion-shift, 22px) * .28), calc(var(--bg-motion-shift, 22px) * -.22), 0) scale(calc(1.08 + (.032 * var(--bg-motion-strength, 1)))); }
          100% { transform:translate3d(0, 0, 0) scale(1.08); }
        }
        .card.background-motion .bg {
          animation:backgroundFloat var(--bg-motion-duration, 38s) ease-in-out infinite;
        }
        .card.background-motion .glow {
          animation:glowDrift var(--glow-motion-duration, 28s) ease-in-out infinite;
        }
        .theme-light .bg {
        ...
        .card.background-motion .hero-aura {
          animation:auraDrift var(--aura-motion-duration, 32s) ease-in-out infinite;
        }
        ...
        .card.background-motion .art-aura {
          animation:auraDrift calc(var(--aura-motion-duration, 32s) * .9) ease-in-out infinite reverse;
        }
...
.theme-light .player-mini-value{color:#435066!important;}
  ...
  _syncNowPlayingUI() {
    ...
    const player = this._getSelectedPlayer();
    const compactTileMode = this._isCompactTileMode();
    const renderCompactTile = ({ title = "", subtitle = "", art = "", icon = "music_note", duration = 0, position = 0, emptyAction = "", upNextItem = null }) => {

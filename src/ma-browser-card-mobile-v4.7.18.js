...
const HOMEII_CARD_VERSION = "4.7.18";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _syncSleepTimerChip() {
    ...
    const chip = this.$("sleepTimerChip");
    const label = this.$("sleepTimerChipLabel");
    const menu = this.$("sleepTimerMenu");
    const remainingLabel = this._sleepTimerFooterLabel();
    const active = !!remainingLabel && this._sleepTimerChipVisible();
    if (!active) this._state.mobileSleepTimerMenuOpen = false;
    if (corner) corner.hidden = !active;
    if (chip) {
      chip.hidden = !active;
      chip.classList.toggle("active", active);
    }
    if (label) label.textContent = remainingLabel;
    if (menu) menu.hidden = !(active && this._state.mobileSleepTimerMenuOpen);
  }
  ...
  _build() {
    ...
    const historyToggleFabHtml = !compactTileMode
      ...
      : ``;
    const sleepTimerChipVisible = this._sleepTimerChipVisible();
    const sleepTimerMenuVisible = sleepTimerChipVisible && !!this._state.mobileSleepTimerMenuOpen;
    const sleepTimerCornerHtml = !compactTileMode ? `
      <div class="sleep-timer-corner ${rtl ? "left" : "right"}" id="sleepTimerCorner"${sleepTimerChipVisible ? `` : ` hidden`}>
        <div class="sleep-timer-menu" id="sleepTimerMenu"${sleepTimerMenuVisible ? `` : ` hidden`}>
        ...
        <button class="sleep-timer-chip" id="sleepTimerChip" title="${this._esc(this._m("Sleep timer", "טיימר שינה"))}"${sleepTimerChipVisible ? `` : ` hidden`}>
          ${this._iconSvg("timer")}
          <span id="sleepTimerChipLabel">${this._esc(sleepTimerLabel)}</span>
        </button>
      </div>
    ` : ``;
...
@media (max-width: 1280px){
  ...
  .control-room-dock-section{padding:6px 8px;}
    ...
    this.$("sleepTimerChip")?.addEventListener("click", (e) => {
      if (!this._pressUiButton(e.currentTarget)) return;
      this._toggleSleepTimerMenu();
    });
    this.$("sleepTimerMenu")?.addEventListener("click", (e) => {

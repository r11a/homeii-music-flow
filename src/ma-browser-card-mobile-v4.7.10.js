...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _controlRoomGridStyle(playerCount = 0) {
    ...
    const height = Math.max(560, Math.round(cardRect?.height || Number(this._config.height || 760)));
    const compactViewport = width <= 1280;
    const gap = compactViewport ? 14 : 18;
    const horizontalPadding = compactViewport ? 40 : 56;
    const verticalPadding = compactViewport ? 86 : 104;
    const footerHeight = compactViewport ? 132 : 146;
    const availableWidth = Math.max(320, width - horizontalPadding);
    const availableHeight = Math.max(220, height - verticalPadding - footerHeight);
    let best = { cols: Math.min(count, compactViewport ? 3 : 4), overflow: Number.POSITIVE_INFINITY, area: 0 };
    for (let cols = 1; cols <= count; cols += 1) {
      const tileWidth = (availableWidth - gap * (cols - 1)) / cols;
      if (tileWidth < 180) continue;
      const tileHeight = tileWidth * 9 / 16;
      const rows = Math.ceil(count / cols);
      const totalHeight = rows * tileHeight + Math.max(0, rows - 1) * gap;
      const overflow = Math.max(0, totalHeight - availableHeight);
      const area = tileWidth * tileHeight;
      const isBetter = overflow < best.overflow - 2 || (Math.abs(overflow - best.overflow) <= 2 && area > best.area);
      if (isBetter) best = { cols, overflow, area };
    }
    return `style="--control-room-cols:${best.cols};--control-room-gap:${gap}px"`;
  }
...
.control-room-dock{position:relative;left:auto;bottom:auto;display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:nowrap;max-width:calc(100% - 8px);width:max-content;margin:0 auto;padding:14px 18px calc(18px + env(safe-area-inset-bottom, 0px));border-radius:28px 28px 0 0;border:1px solid rgba(255,255,255,.14);border-bottom:none;background:rgba(9,12,18,.52);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);box-shadow:0 10px 28px rgba(0,0,0,.22);transform:none;align-self:end;z-index:2;}

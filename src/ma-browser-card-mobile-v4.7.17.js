...
.card.layout-tablet .active-player-chip .bars,.card.layout-tablet .active-player-card .bars{display:none!important;}
  ...
  _syncRecentHistoryUi() {
    ...
    const nextHtml = `
      ...
      `).join("")}
    `;
    if (this._state.mobileHistoryRenderedHtml !== nextHtml) {
...
const HOMEII_CARD_VERSION = "4.7.17";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _build() {
    ...
    const sleepTimerCornerHtml = !compactTileMode ? `
      <div class="sleep-timer-corner ${rtl ? "left" : "right"}" id="sleepTimerCorner">
        <div class="sleep-timer-menu" id="sleepTimerMenu" hidden>
        ...
        <button class="sleep-timer-chip" id="sleepTimerChip" title="${this._esc(this._m("Sleep timer", "טיימר שינה"))}">
          ${this._iconSvg("timer")}

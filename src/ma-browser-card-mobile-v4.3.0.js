...
const HOMEII_CARD_VERSION = "4.3.0";
...
class MABrowserCardMobile extends MABrowserCard {
  ...
  _isNightModeActive(date = new Date()) {
    ...
    return this._isMinutesInsideWindow(
      nowMinutes,
      this._clockMinutesOfDay(windowRange.start, "22:00"),
      this._clockMinutesOfDay(windowRange.end, "06:00")
    );
  }
  ...
  _syncNightModeUi() {
    ...
    if (card) {
      ...
      card.classList.toggle("night-mode-enabled", mode !== "off" || sleepActive);
    }
    const row = this.$("nightQuickRow");
    if (row) row.hidden = !(mode !== "off" || sleepActive);
    const modeBtn = this.$("nightModeQuickBtn");
    const modeLabel = this.$("nightModeQuickLabel");
    if (modeBtn) {
      modeBtn.classList.toggle("active", active);
      modeBtn.classList.toggle("soft", !active);
    ...
    if (modeLabel) {
      modeLabel.textContent = mode === "auto"
        ? active ? this._m("Night auto", "לילה אוטומטי") : this._m("Auto night", "לילה אוטומטי")
        : mode === "on"
          ? this._m("Night on", "לילה פעיל")
          : this._m("Night off", "לילה כבוי");
    }
    const sleepBtn = this.$("nightSleepBtn");
    const sleepLabel = this.$("nightSleepLabel");
    if (sleepBtn) {
    ...
    if (sleepLabel) {
      sleepLabel.textContent = sleepActive
        ? `${this._m("Sleep", "שינה")} ${this._sleepTimerRemainingLabel()}`
        : this._m("Sleep", "שינה");
    }
    const chillBtn = this.$("nightChillBtn");
    if (chillBtn) {
      chillBtn.hidden = !(mode !== "off" || active);
    }
  }
  ...
  _build() {
    ...
    const showNightRow = nightMode !== "off" || sleepTimerActive;
    const height = Number(this._config.height || 760);
    const nightQuickRowHtml = `
      <div class="night-quick-row" id="nightQuickRow" ${showNightRow ? "" : "hidden"}>
        <button class="night-quick-btn ${nightActive ? "active" : "soft"}" id="nightModeQuickBtn" title="${this._esc(this._m("Night mode", "מצב לילה"))}">
          ${this._iconSvg("moon")}
          <span id="nightModeQuickLabel"></span>
        </button>
        <button class="night-quick-btn ${sleepTimerActive ? "active" : ""}" id="nightSleepBtn" title="${this._esc(this._m("Sleep timer", "טיימר שינה"))}">
          ${this._iconSvg("timer")}
          <span id="nightSleepLabel"></span>
        </button>
        <button class="night-quick-btn soft" id="nightChillBtn" title="${this._esc(this._m("Chill mix", "מיקס רגוע"))}">
          ${this._iconSvg("wand")}
          <span>${this._esc(this._m("Chill", "Chill"))}</span>
        </button>
      </div>`;
...
.theme-light .player-mini-value{color:#435066!important;}
        ...
        <div class="settings-group">
          ...
          <div class="settings-color-wrap">
            ...
            <div class="settings-color-row">
              <input class="settings-text-input" id="mobileNightStartInput" type="time" value="${this._esc(nightWindow.start)}">
              <input class="settings-text-input" id="mobileNightEndInput" type="time" value="${this._esc(nightWindow.end)}">
            </div>
          </div>
          <div class="settings-hint">${this._m("Auto mode turns on between these hours. The range can cross midnight.", "מצב Auto נכנס לפעולה בין השעות האלה, גם אם הטווח חוצה חצות.")}</div>
  ...
  _handleMobileMenuChange(e) {
    ...
    if (e.target?.id === "mobileNightStartInput") {
      this._state.mobileNightModeStart = this._normalizeClockTime(e.target.value || "22:00", "22:00");
      this._persistMobileAppearance();
      this._build();
      this._init();
      this._openMobileMenu("settings");
    ...
    if (e.target?.id === "mobileNightEndInput") {
      this._state.mobileNightModeEnd = this._normalizeClockTime(e.target.value || "06:00", "06:00");
      this._persistMobileAppearance();
      this._build();
      this._init();
      this._openMobileMenu("settings");
